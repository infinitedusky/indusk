...
- **Plan hierarchy is declared top-down, in frontmatter**: the root `master.md` names `parents:` + the `roadmap:` order; each parent's own `master.md` names its ordered `subplans:`. Children declare nothing — one source of truth per link, so the two sides can't drift. **The plan inventory always comes from disk** (`parseAllPlans`), never from a list; declarations add structure and can never subtract a plan. A missing/corrupt/absent key degrades to the flat list. `readPlanDeclarations` in `apps/indusk-mcp/src/lib/plan-parser.ts`. — see `/reference/cli/plans`
...
- **The admin sidebar tree is derived entirely from declarations** — a plan appearing at top level means no parent claims it, not a reader bug. Grouping lives in `PlanList.tsx` (`buildGroups`), not the reader, which stays a pure data layer; every plan in comes back out (group or `rest`). A browser test that mocks `@/lib/planning-reader` must include every export the layout imports, or the mock fails with "does not provide an export named …". A plan whose documents all fall outside `DOC_FILES` (a parent carrying only `master.md`) renders empty unless the detail view branches for it — `PlanDetail` renders parents as subplan cards + master prose (`subplans` prop from the page), and doc-less plans header-only.
...
| graph-knowledge-architecture / cursor-support / indusk-v2-dawn / midnight | research/parked | **review against the makeover** — the graph-canonical direction is rejected; these need re-scoping or archiving |
