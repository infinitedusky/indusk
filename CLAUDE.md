...
- **Working agent writes highlights via the `highlight` MCP tool instead of calling `graph_capture` directly.** Highlights land in `.indusk/highlights.jsonl` (append-only) with a level (`critical` / `important` / `note`), a tag, and a short note. The eval agent reads unprocessed highlights via `highlights_unprocessed`, writes structured Graphiti episodes with level-weighted edges, and marks each entry done via `highlight_mark_processed`. Working-agent trigger points (brief-accepted, adr-accepted, correction, retro-lesson) will migrate from direct `graph_capture` calls to `highlight` calls in Phase 2 of the agent-roles plan.
