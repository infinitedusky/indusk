...
- **Thin-lane eval rail**: `atdawn run` cannot spawn the evaluator (it may run where no `claude` CLI exists), so each loop commit appends to `.indusk/eval/pending.jsonl`; `eval-trigger.js --drain-pending` evaluates each exactly once (ledger written *before* spawn — a crash is a gap, never a double-eval), `/rail-check` owns the drain, `check_health` surfaces the backlog. Eval state is excluded from work-product staging. **A drain never discards a record whose evaluator failed** — the ledger entry is provisional until the evaluator exits 0, so a machine that cannot evaluate leaves the backlog intact and says so; durability covers evaluator failure, not just crashes. Commit messages account for every item a commit contains (batched checkoffs and items carried from a failed attempt included). — see `.indusk/planning/dawn-hook-parity/adr.md`
