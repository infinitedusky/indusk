...
- Context system evaluation via commit-triggered judge agent: spawns read-only Opus agent in background on every `jj describe`, does full catchup, reads transcript + diff, answers evaluation questions, logs structured scorecards. Questions are the rubric (start with 4, grow over time). Judge knows the outcome so it audits backward from certainty — see `.indusk/planning/semantic-graph-eval/adr.md`
...
|------|-------|-----------|
| semantic-graph-eval | impl (in-progress) | End-state integration testing, then retrospective |
