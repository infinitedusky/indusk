...
- **`indusk run <plan> --model <name>`** (Dawn external orchestrator, `apps/indusk-mcp/src/lib/run/`) — plan phases through a model-agnostic gated AI-SDK loop: per-phase scope, advance-on-green via a deliberate `check-gates` probe, goalpost guard, human-gate pause; direct provider keys via the registry. — see `/reference/cli/run`
