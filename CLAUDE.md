...
- **The verify ledger's failure-safety is the deliberate INVERSE of the pending-eval ledger's.** Pending-eval writes its done-marker *before* the risky spawn (crash = gap, never double-eval); `.indusk/verify/ledger.jsonl` appends *only after* a clean verdict, because the danger there is a rejected phase silently becoming the baseline the next phase is judged against. `readLedger` **throws** on a malformed line rather than skipping it — a skipped line shortens the chain, which degrades into bootstrap mode and yields a confident report against the wrong baseline (a failure indistinguishable from success). Never "repair" it by filtering.
...
- **indusk-makeover (Phases 0–6, close-out pending)** — budgets + decay + Graphiti/CGC removal + catchup diet + hub sync; measured 144 KB → ≤60 KB and ~55k → ~8.2k catchup tokens. Numero-workbench migrates after publish. **Known open: `agent-roles-phase4` fails on main** — it asserts a Key Decisions bullet this plan's compaction removed. See `.indusk/planning/indusk-makeover/`.
