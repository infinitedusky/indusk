...
- [x] **H19 fix (T10)**: in `apps/indusk-mcp/src/lib/highlights/highlights.ts`'s `markProcessed`, added a defensive substring check on the RAW file content via a new `isIdInRawProcessed(projectRoot, id)` helper. After the parsed-content `readAllProcessed.find` check returns no match, the defense fires: if the raw file content contains `"id":"<escaped id>"` (JSON-safe regex), return `{ already_processed: true, original_processedAt: undefined }` without appending. The boolean signal is what triggers the agent's STOP path; original_processedAt is undefined because the malformed entry didn't yield a parseable timestamp.
- [x] **Optional helper**: extracted as `isIdInRawProcessed(projectRoot: string, id: string): boolean` — file-private (not exported) since the contract is internal to the dedup defense. Escapes regex metacharacters in the ID defensively (highlight IDs are alphanumeric + hyphens in practice, but defensive escaping is cheap).

#### Phase 7 Verification
- [x] T10 passes — `highlights.test.ts` extended with two cases: (1) writes a malformed truncated-JSON line for `h-20260101-001`, calls `markProcessed`, asserts `already_processed: true` AND file line count stays at 1; (2) confirms `readAllProcessed` still silently skips the malformed line (parser tolerance preserved — the defense works around it on the WRITE side without changing the parser's read-side tolerance).
- [x] Phase 7 verification also confirms the surface symptom is preserved — `readAllProcessed` still returns an empty array for the malformed case; `readUnprocessedHighlights` falls back to "not processed" since no well-formed highlights exist either. The contract specifically defended is on the WRITE side.
- [x] Full `pnpm vitest run` from `apps/indusk-mcp/` passes — 667 passed, 11 skipped (vs 665 pre-Phase-7; +2 new Phase 7 tests).
