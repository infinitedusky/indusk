...
| T10 | a phase-checkoff performed through the `bash` tool (`sed -i` / `echo >>` / `cat >`) is gated identically to the `edit` tool — the premature checkoff does NOT survive | Phase 0 | Phase 6 | passing |
| T11 | the `bash` tool cannot mutate files outside the worktree root (absolute path, `cd ..`, `~`) — the attempt is refused, not merely started in the root | Phase 0 | Phase 6 | passing |
| T12 | a symlink inside the worktree pointing outside it cannot be used by the file tools to read or write beyond the root | Phase 0 | Phase 6 | passing |
| T13 | flipping a non-terminal trajectory row to `skipped`/`blocked` mid-phase does NOT let the phase-close probe advance — self-assigned terminality is a moved goalpost | Phase 0 | Phase 6 | passing |
...
| T17 | behavior parity across the Phase 7 decomposition — the extracted modules are imported by their consumers and the full `src/lib/run` suite passes with unchanged assertions | Phase 7 | Phase 7 | passing |
...
- [x] Gate the `bash` write surface (T10): snapshot the gate-relevant files (the impl under run, at minimum) before each `bash` call; if the command mutated one, run the same `{tool_name, tool_input, cwd}` envelope against the resulting change and REVERT + return the block message when the gate refuses. Today `bash("sed -i 's/- \[ \]/- [x]/' impl.md")` checks off a phase with zero gate involvement — the one move a blocked model is most likely to reach for.
- [x] Confine `bash` to the worktree (T11): `cwd: root` sets a starting directory, not a boundary — absolute paths, `cd ~`, and `..` all escape today while the file tools carefully reject exactly that. Add a best-effort guard AND state plainly (code comment + docs) that real confinement needs the sandboxed run cell (roadmap); never let the guard imply isolation it does not provide.
- [x] Harden `resolveInWorktree` against symlink escape (T12): it normalizes textually via `resolve()` and never calls `realpath`, so a symlink inside the root that points outside passes the check and the write lands outside. Resolve the real path (of the file, or its nearest existing parent for new files) before the containment comparison. Note the compounding path: `bash` can *create* the symlink, then the "safe" file tools walk through it.
- [x] Close self-assigned terminality (T13): `checkGoalposts` permits every State transition, and `TERMINAL_STATES` includes `skipped`/`blocked` — so a model that cannot make a test pass can write `skipped` into the State cell and the phase-close probe will happily advance. Treat non-terminal → `skipped`/`blocked` *inside a phase* as a goalpost violation (STOP LOUD); human-authored skips outside the loop stay legal. Also flag `Writable at` moved later, which the guard ignores today.
- [x] Make the gate invoker fail LOUD, not open (T14): `runGateScripts` blocks only on exit 2 and allows everything else "for PreToolUse parity" — but Claude Code has a human watching, and this loop does not. A gate script that crashes (or vacuously passes a zero-phase impl — the known `validator-must-reject-zero-parsed-phases` lesson) silently disarms enforcement for the rest of the run. Treat non-zero-non-2 as a hard stop that surfaces the script's stderr and halts the loop, rather than a quiet allow.
- [x] Treat a killed gate script as a block (T15): `spawnGateScript` has `timeout: 30_000`; on kill the exit code is `null`, which is `!== 2`, which means **allow**. A loaded machine can therefore disarm the gate — and this is not theoretical: Phase 3 observed 5s gate-spawn timeouts under background load. Block on null exit with an explicit "gate script timed out — refusing the edit" message, and make the timeout configurable.
- [x] Validate `--max-steps` (T16): `Number.parseInt` yields `NaN` for `--max-steps abc`, and `stepCountIs(NaN)` never fires — the loop's only cost bound silently disappears. `0`/negative give an instant zero-work red. Reject non-numeric and `<1` at the CLI boundary with a clear message.

#### Phase 6 Verification
- [x] T10: a `bash`-driven premature checkoff on the guinea-pig impl is gated identically to the `edit` path — the checkoff does not survive the call. Green, and paired with a CONTROL test proving the same `sed -i.bak` command DOES mutate an ungated worktree (the first draft passed vacuously because the shell command was broken — false pass caught before it could mislead).
- [x] T11: a `bash` command writing to an absolute path outside the worktree root is refused.
- [x] T12: a symlink inside the worktree pointing outside it cannot be used by `readFile`/`writeFile`/`edit` to cross the root.
- [x] T13: flipping a non-terminal row to `skipped` mid-phase is caught as a goalpost violation and the loop STOPS instead of advancing (with a control proving `written → passing` stays legal).
- [x] T14: a gate script exiting 1 (crash/malformed impl) stops the run loud instead of allowing the edit.
- [x] T15: a gate script killed by timeout blocks the edit with a timeout-specific message.
- [x] T16: `--max-steps abc` / `--max-steps 0` are rejected with a clear error before any model call (asserted on the max-steps-specific message so it cannot pass via the key check).
- [x] Full suite: `pnpm vitest run src/lib/run/` → 6 files, 51/51 passing; `pnpm build` (tsc) clean; `biome check` clean. Wider package suite has 10 pre-existing failures (admin daemon/bundle + a CLAUDE.md assertion) — verified identical on the pre-change baseline via `git stash`, none introduced here.

#### Phase 6 Context
- [x] Add a Known Gotcha to CLAUDE.md recording the enforcement boundary this ritual exposed: the orchestrator's gate covers the `edit`/`writeFile` tools, so any *new* tool that can mutate files (starting with `bash`) must be routed through the same envelope or it is a hole in Tier-1 by construction.
...
- [x] Extract the shared run-test harness into `src/lib/run/__tests__/harness.ts` — `hooksDir`, `realGateScripts`, `fixtureDir`, `executeOf`, and a `makeGatedWorktree()` that mkdtemps a root, copies `.claude/hooks` + the guinea-pig fixture, and returns cleanup. Basis: rule of three, exceeded — `gate.test.ts`, `falsification.test.ts`, `loop.test.ts`, and `swap.test.ts` each redeclare the same four constants, and three repeat the same temp-worktree setup verbatim. One harness, four consumers.
- [x] Split `src/lib/run/loop.test.ts` (470 lines, over the 400 cap) so tests live beside the modules they now target — goalpost tests to `goalposts.test.ts`, probe tests to `probe.test.ts`, leaving `loop.test.ts` for T5's full-loop and human-gate behavior. Basis: tests mirror source structure; the file only exceeds the cap because it currently covers three modules' worth of behavior.
- [x] (reviewed `src/bin/cli.ts` — left as-is: 706 lines, but this plan added ~15 of them (the `run` subcommand registration + the invoked-name branding). Decomposing a 700-line command registry is a legitimate refactor and an illegitimate rider on this plan; it belongs to its own plan with its own trajectory.)
- [x] (reviewed `driver.ts` 173, `registry.ts` 130, `run.ts` 158, `tools.ts` — left as-is: each is one cohesive unit well under the 400-line cap. `tools.ts` drops to ~145 after the path extraction and is otherwise just five tool definitions.)
- [x] (reviewed `gate.test.ts` 291, `swap.test.ts` 347, `falsification.test.ts` 237 — left as-is: all under cap, and each shrinks further once the shared harness lands. Splitting them further would scatter a single trajectory row's evidence across files.)

#### Phase 7 Verification
- [x] T17: behavior parity across the decomposition — every extracted module is imported by its consumers, and the full `src/lib/run` suite passes with its assertions unchanged. `pnpm vitest run src/lib/run/` → 8 files, **55/55 passing** (51 pre-existing + 4 new seam assertions); `tsc --noEmit` clean; `biome check` clean. Post-split sizes: gate.ts 400→282, loop.ts 400→239, tools.ts 198→142, loop.test.ts 470→400 — every file in `src/lib/run` now under the 400 cap. New modules: `worktree-paths.ts` (68), `bash-gate.ts` (132), `probe.ts` (114), `goalposts.ts` (76), `harness.test-support.ts` (77), `goalposts.test.ts` (83).

#### Phase 7 Context
- [x] (none needed — internal decomposition; CLAUDE.md's Architecture entry points at `apps/indusk-mcp/src/lib/run/` as a directory, which stays accurate, and the Phase 6 gotcha describes the enforcement boundary, not the file layout)

#### Phase 7 Document
- [x] (none needed — no public surface changes: same CLI, same flags, same exported behavior; `/reference/cli/run` describes the contract, not the module structure)
