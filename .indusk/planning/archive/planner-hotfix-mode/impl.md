...
- [x] Anchored `check-gates.js`'s `detectWorkflow` regex to `/^workflow:\s*(bugfix|refactor|feature|spike|hotfix)/m` (both source and installed `.claude/hooks/` copy).
- [x] Anchored `validate-impl-structure.js`'s equivalent regex the same way (both copies).

#### Phase 4 Verification
- [x] T8: the substring-in-title case no longer misdetects; the real `workflow:` YAML key still correctly detects the true value. 2 new assertions in `planner-hotfix-mode.test.ts` (validate-impl-structure.js's write-time behavior + check-gates.js's gate-requirement behavior, using a discriminating fixture with an unchecked Context item since check-gates.js's model is item-existence-based, not section-presence-based) — 8/8 total tests passing.

#### Phase 4 Context
- [x] Extended the existing workflow-dispatch-duplication Known Gotcha in CLAUDE.md with the anchoring fix + rationale, cross-referencing the `rationale_baseline` precedent.

#### Phase 4 Document
- [x] The CLAUDE.md Known Gotcha doubles as the user-facing documentation for this fix. No separate docs-site page — internal hook-correctness fix, not a new capability.
