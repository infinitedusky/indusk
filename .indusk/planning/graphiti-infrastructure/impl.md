...
- [x] **MCP registration** — `.mcp.json` in this repo contains the `graphiti` entry pointing at `http://localhost:8100/mcp`. Verified at the start of this session via `cat .mcp.json | jq` (5 servers including graphiti). On a fresh project, `indusk init` would register it automatically (the init.ts code path is in place from Phase 5.5 implementation).
- [x] **Tool exposure** — Confirmed at session start that `mcp__graphiti__add_memory`, `mcp__graphiti__search_nodes`, `mcp__graphiti__search_memory_facts`, `mcp__graphiti__get_episodes`, `mcp__graphiti__get_status` are all callable from this session. `get_status` returned `{status: "ok", message: "Graphiti MCP server is running and connected to falkordb database"}`.
