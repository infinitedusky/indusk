...
- [ ] **H14 fix (T5)**: in `persistent-evaluator.ts`'s resume-prompt construction, replace `"answer the same evaluation questions as before"` with `"answer the rubric questions"` (or `"answer the v1 rubric questions"`). The "as before" phrasing creates ambiguity with the 197-turn conversation history where Step 4 was never present — the inner Claude reads it as "your last turns" and skips Step 4 to match. Drop the backwards-anchoring temporal modifier entirely.
- [ ] **H15 fix (T6)**: in `prompt-builder.ts`'s `buildHighlightsInstructions` helper, add a sentence immediately after the existing `If \`mcp__indusk__highlights_unprocessed\` is unavailable, skip this step silently and continue.` line: `If the tool returns an empty list (no unprocessed highlights), log "(no unprocessed highlights)" in your output briefly and continue to the rubric — do not invent highlights, do not loop searching for them.` This pins the empty-queue path so once the backlog drains, subsequent commits don't get stuck or hallucinate.
- [ ] **H16 fix (T7)**: add a source-grep regression test in the existing `apps/indusk-mcp/src/__tests__/eval-resume-prompt-includes-highlights.test.ts` (or a new file) asserting `persistent-evaluator.ts` source contains BOTH the `--mcp-config` literal AND the `bypassPermissions` literal. The April 1.23.x fix is the foundation everything else builds on — without these flags the inner Claude has no MCP tool surface and Step 4 fires into a void.

#### Phase 5 Verification
- [ ] T5 passes — source-grep extends the Phase 4 test file (or new sibling) asserting the resume-prompt construction does NOT contain `"as before"` (or `"same evaluation questions"` qualified with `"before"` anywhere in the resume branch).
- [ ] T6 passes — source-grep on `prompt-builder.ts` for the helper text matches an "empty list" / "no unprocessed highlights" branch instruction; pre-fix the only branch instruction is for "unavailable."
- [ ] T7 passes — source-grep on `persistent-evaluator.ts` asserts presence of both `--mcp-config` AND `bypassPermissions` literals. Goes red if either is removed.
- [ ] Full `pnpm vitest run` from `apps/indusk-mcp/` passes (no regression on the 658-test suite).

#### Phase 5 Context
- [ ] Append a sentence to the CLAUDE.md "eval agent resume prompt must include Step 4" gotcha (added in Phase 4) naming the three Phase 5 invariants: no backwards-anchoring phrasing in the commit-evaluation line, explicit empty-list handling in the helper text, and the source-grep regression on the MCP-access spawn flags.

#### Phase 5 Document
- [ ] Append a "Phase 5 falsification fixes (1.31.2)" entry to the changelog under `[Unreleased]` describing the three falsification-surfaced fixes: phrasing change, empty-queue handling, spawn-flag regression. Bump `apps/indusk-mcp/package.json` from `1.31.1` to `1.31.2` since the Phase 4 fix (1.31.1) hasn't been published yet — the Phase 5 fixes ship with it as 1.31.2 OR ride along on 1.31.1 if 1.31.1 hasn't shipped. The work skill will decide based on publication state.
