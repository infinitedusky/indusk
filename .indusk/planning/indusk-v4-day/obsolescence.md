...
Companion piece to [thesis.md](thesis.md), broken out from what began as an
aside. The thesis says AI will replace a lot of engineers and should. This
piece looks at the future people are actually afraid of, and asks what causes
it, because the cause is not the one people assume.

## Two futures, and only one is close
...
## The cause of the feared future
...
speed, and that gap, not the one between engineers and their tools, is the
one that determines which of the three futures arrives.

## It is easier to be an electrician now
...
The three-dimensional plan, vetted before work starts, is the contract: the
claims, in plain language, approved before the code exists. The customer
putting input directly into the plan is the reviewer reading claims and
asking *what about this?* without reading a line of code. The craft done by
feel is the vibe coding that everyone does now and should stop being ashamed
of. And the barrier to entry falling is the thesis's line that neither side
has to read code: more people can do this work, because the part that
required years of reading is now done by the shape.

The engineer who is afraid is afraid of reading one. Reading one is a choice
made by someone else. The work worth doing is making readings two and three
the ones that happen, and the way to do that is not to slow the engineer down
until the old business can keep up. It is to build the businesses that can
keep up with the engineer.
