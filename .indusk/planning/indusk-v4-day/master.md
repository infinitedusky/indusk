...
| 4a | **Promises — the contract's primitive: registry, kinds, states, links, the check** | 9 | brief rewritten 2026-09-18 on the settled frame (looper has the registry and validator, 11 promises; numero has the typed contract and no promises; dusk has dozens of unregistered structure promises) | The check refuses every way the registry can lie, by name, and passes looper's eleven as a fixture; dusk holds three, one per kind, with the check in its suite; the Promises page lists them with every enforced chip hollow. Looper and numero are downstream and adopt after publish, in their own plans | [day-promises](../day-promises/brief.md) |
...
|------|-----------|-------|
| `day-promises` | 4a — the promise as a primitive: registry, kinds, lifetimes, states, links, `indusk promises check`, dusk self-hosting, the Promises page (looper and numero adopt downstream) | brief accepted, test plan accepted, ADR proposed 2026-09-18 |
