...
| 2 | **The floor runs in workbenches** | 5 | **impl complete 2026-09-16** (15 rows green; close-out rituals pending) — `run` and `verify` execute across the plan-root/code-root split for one declared repo, evals name their repo; matrix held 5/5, 0 false positives (`dawn-workbench-execution/matrix.md`) | dawn-verify's 6-cell matrix re-run inside a workbench, 5/5 caught, 0 false positives | [dawn-workbench-execution](../dawn-workbench-execution/brief.md), Dawn 6.5 |
