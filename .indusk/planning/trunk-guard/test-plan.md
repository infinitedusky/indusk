...
| A5 | `indusk init` registers `trunk-guard.js` under both the `Edit\|Write` and the `Bash` PreToolUse matchers; `indusk update` on a project whose settings lack them adds both; a second `update` changes the settings file by zero bytes | vitest, `runCli` (`INDUSK_HOME` pinned by the helper) |
