...
| 6.5 | **Workbench execution** — the floor runs where the work is | **Brief drafted 2026-09-03** | Run + verify + eval attribution across the plan-root/code-root split, for single-repo workbenches (multi-repo keeps refusing). Found by the 2026-09-03 audit: `atdawn run` cannot reach the code in a workbench and commit-cadence's disable guard became unreachable when roots became git repos — see `.indusk/planning/workbench-trust-fixes/research.md` F1. Trust-fixes (refusals) land first | The dawn-verify 6-cell matrix re-run **inside a workbench** — 5/5 caught, 0 false positives — and `atdawn run` takes a plan end-to-end in all four workbench layouts | [dawn-workbench-execution](../dawn-workbench-execution/) |
...
Component 6's verify is also the **universal floor**: it runs on every tier as detection defense-in-depth. **Proven 2026-08-05 — in flat repos.** In a workbench it deliberately refuses (plan and code are different repos; the ledger's baseline has no meaning across them), so the floor is not yet under the standard dev shape — component 6.5 lifts that split — and the floor turned out to matter on tiers 1 and 2 as much as tier 3, because the trajectory's `passing` state had never been checked against a test run in *any* lane.
...
| [dawn-workbench-execution](../dawn-workbench-execution/) | 6.5 — workbench execution | brief draft 2026-09-03 |
