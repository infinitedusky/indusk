...
| 2 | **Gate portability** — InDusk's hooks enforce outside Claude Code | **Done — closed 2026-08-03** | Every invariant hook now enforces in the thin lane (`claude-md-budget` wired; `eval-trigger` served by the pending-eval queue + external drain); `ask` is the default in both lanes with a real headless pause; `gate-reminder` **deliberately shed** (advisory nudge, not an invariant — first keep-shed audit entry). Inventory was 5 hooks at close; 1.37.0 added `workbench-sync.js` (6 on disk), unrecorded in the keep/shed audit until the 2026-09-03 audit — now classified **shed** (sync cadence: a procedure, not an invariant gate). **Keep/shed record, all seven** (pinned by `hooks-record-parity.test.ts`): keep — `validate-impl-structure`, `check-gates`, `claude-md-budget` (write-path gates), `trunk-guard` (the trunk gate, 2026-09-17: no code edited or committed on `main`; the thin lane runs on a plan branch by construction, so its lane parity is "never on trunk", enforced by the same hook when the loop's Bash lane commits), `eval-trigger` (the rail); shed — `gate-reminder` (advisory), `workbench-sync` (procedure). Shipped 1.36.0 | A fresh plan executes identically under both lanes, hook for hook, for every hook that is an invariant; sheds are recorded | [dawn-hook-parity](../archive/dawn-hook-parity/) |
...
| 6 | **Verification of work Dawn didn't do** | **Done — acceptance met 2026-08-05** | `atdawn verify <plan> --phase N` ships five detections (premature checkoff, skipped test-first duty, goalpost drift, **red tests**, **phantom work**) over a chained verify ledger. Detects and reports only — reverting is component 7's. Shipped 1.36.0. **Refuses in every workbench** (deliberate — `verify/roots.ts`); lifting that split is component 6.5 | **Met.** 6-cell matrix against a hookless `claude` session (hooks on disk, unregistered — the Cursor shape): 5/5 planted classes caught, 0 misses, 0 false positives on the honest control. See [matrix.md](../archive/dawn-verify/matrix.md) | [dawn-verify](../archive/dawn-verify/) |
| 6.5 | **Workbench execution** — the floor runs where the work is | **Done — closed 2026-09-16** (falsified 3, cleaned, retrospective, archived) | One `resolveExecutionRoots` behind run, verify and the cleanup scan; verify judges the code repo with a `codeSha` baseline; the loop carries a plan root and a code root with a commit cadence per repo (`Code-Commit:` trailer); queued evals name their repo. Multi-repo keeps refusing. Found by the 2026-09-03 audit (`archive/workbench-trust-fixes/research.md` F1); trust-fixes' refusals landed first and were lifted case by case | **Met 2026-09-16.** The 6-cell matrix re-run inside a workbench with a headless uncontrolled agent: honest control clean, 5/5 planted classes caught, 0 false positives (`archive/dawn-workbench-execution/matrix.md`); the scripted loop takes the guinea-pig plan end-to-end on all four one-repo layouts (A13) | [dawn-workbench-execution](../archive/dawn-workbench-execution/) |
...
**1 → 3 → 2 → 6 → 6.5 → 7 → 8**, with 5 pulled in whenever the always-on box is actually wanted, and 4 settled as a decision rather than built.
...
Component 6's verify is also the **universal floor**: it runs on every tier as detection defense-in-depth. **Proven 2026-08-05 in flat repos, and 2026-09-16 inside a workbench** (component 6.5: a one-repo workbench resolves to a plan root and a code root, verify judges the code repo against a `codeSha` baseline, and the matrix held there too). Only a workbench declaring several repos still refuses, by name. The floor turned out to matter on tiers 1 and 2 as much as tier 3, because the trajectory's `passing` state had never been checked against a test run in *any* lane.
...
- **Relationship to Midnight.** Midnight's brief (rewritten 2026-08-28) names this master as its open-question counterpart: does the promise/collapse signal feed `indusk run` — the missing feedback edge for unattended loops — or are the two orthogonal (Dawn = who executes, Midnight = what authority a test has)? **Midnight's ADR settles it**; if it feeds, components 5/7 consume the signal.
...
| [dawn-workbench-execution](../archive/dawn-workbench-execution/) | 6.5 — workbench execution | closed 2026-09-16 — 19 rows green, falsified, cleaned, retrospective, archived |
...
**Correction (2026-09-03):** a full audit against the versioned-workbench era found this file stale while claiming to be the answer to "where are we": the word *workbench* appeared nowhere in it while every real project became one, and components 1–3's Done did not hold in that shape (`atdawn run` mis-commits silently there — `.indusk/planning/workbench-trust-fixes/research.md`, F1). Component 6.5 and the trust-fixes plan are the response. A living master stays living only if work that changes the ground it stands on updates it — versioned-workbench's retrospective never cross-referenced this plan, and nothing forced it to.
