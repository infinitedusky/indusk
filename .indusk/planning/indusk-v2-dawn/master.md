...
|---|-----------|--------|--------------|-----------------|----------|
| 1 | **Model-agnostic execution** — call any provider's model through one loop | **Done** | Claude driver never exercised live (no API key); `gpt`/`grok` resolve but have no factory | `atdawn run <plan> --model X` takes a plan to impl-complete | [dawn-external-orchestrator](../dawn-external-orchestrator/) |
