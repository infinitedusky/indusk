...
| 2 | **Gate portability** — InDusk's hooks enforce outside Claude Code | **In flight** | Corrected inventory (research 2026-08-03): **5 hooks on disk, not 6** — `check-plan-order.js` was deleted in the context-beam cleanup (`62186774`). At plan start 3 unwired: `claude-md-budget` (now wired), `eval-trigger` (queue+drain design), `gate-reminder` (**deliberately shed** — advisory nudge, not an invariant; see the plan ADR). `gate_policy` headless-`ask` pause in flight | A fresh plan executes identically under both lanes, hook for hook, for every hook that is an invariant; sheds are recorded | [dawn-hook-parity](../dawn-hook-parity/) |
