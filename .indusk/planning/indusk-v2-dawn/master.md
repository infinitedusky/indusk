...
|---|-----------|--------|--------------|-----------------|----------|
| 1 | **Model-agnostic execution** — call any provider's model through one loop | **Done** | Claude driver smoke-verified live 2026-08-03 (key in `~/.indusk/config.env`; note `atdawn run` reads only `process.env` — source config.env or wire the loader). A8 unblocked. `gpt`/`grok` resolve but have no factory | `atdawn run <plan> --model X` takes a plan to impl-complete | [dawn-external-orchestrator](../dawn-external-orchestrator/) |
