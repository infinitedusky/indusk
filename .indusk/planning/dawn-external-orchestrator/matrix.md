...
| C6 | claude-sonnet-4-5 | local, **Claude Code harness** (fresh-context subagent, same phase contract) | ✅ (hooks live; no blocks needed) | **impl-complete, first attempt** | 1 | 2m06s | 68,037 (harness accounting — cached/counted differently than the AI-SDK cells; not directly comparable) | ~21 tool calls; test-first honored (red run before source); all files inside the fixture dir; 14 tests vs C5's 27; checked all 7 boxes incl. the two `(none needed)` gates (C5's loop closed green with those at 5/7); 14/14 green, independently verified |
...
- **F8 (2026-08-03)**: the first harness-vs-harness pair (C5 vs C6 — same model, same task, same starting state; the A8 shape, at sonnet rather than Opus). **Process parity held**: both cells impl-complete first attempt, both test-first-red unprompted, both layout-clean, comparable wall clock (2m38s vs 2m06s). The deltas were *thoroughness direction reversed from expectation*: the thin atdawn loop produced ~2× the test assertions (27 vs 14) and hand-probed the CLI harder, while the Claude Code harness was more meticulous about *bookkeeping* (7/7 boxes vs 5/7 — atdawn's probe accepts `(none needed)` gates unchecked). Token figures are not comparable across harnesses (raw AI-SDK in/out vs Claude Code's cached accounting). Single sample each — directional only; the Opus pair remains the definitive A8 cell if wanted. (plain image, no volume) — the reprovisioned box lost node/pnpm/key on restart. Also: nodesource started 403ing from the Fly IP on the second pass (fallback: official nodejs.org tarball, more reliable anyway). Consequence for cloud-deployment: the box needs a bootstrap script, a baked image, or a volume — exactly the workspace-lifecycle layer RDEs productize; this is the first concrete datum for the Fly-vs-RDE evaluation.
