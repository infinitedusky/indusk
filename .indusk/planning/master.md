...
| I.4 | [workbench-mode-rail-integrity](workbench-mode-rail-integrity/brief.md) | brief accepted, impl drafted 2026-06-28 | **TOP PRIORITY**. Eval→Graphiti pipeline structurally broken on workbench-shaped projects since worktree-extension shipped — numero_workbench has 86 unprocessed highlights and 0 Graphiti episodes across 2 months. Hooks walk up to `.indusk/` (workbench root, not a git repo), then run `git rev-parse` there and bail. Three pieces: workbench-aware path resolution in 4 hooks + stray-state audit + one-shot backfill of 86. Also a retroactive falsification finding for indusk-worktree-extension (I.2). Targets indusk-mcp 1.31.7. |
