...
| 3.8 | [code-reviewer-agent](code-reviewer-agent/impl.md) | impl draft (sibling of eval agent: post-commit reviewer fires on `jj describe`, anchored to CLAUDE.md/AGENTS.md conventions, MVP scope = cleanliness/DRY/large-function rules; tests-red-first trajectory; opt-in via `review.enabled: false` default) | — | (additional commit-time signal source; future Dawn-petal candidate) |
