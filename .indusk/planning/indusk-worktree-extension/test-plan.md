...
date: 2026-05-20
status: draft
revision: 3 (2026-05-20 — workbench-only narrowing; single-repo shape dropped; dogfood matrix becomes two workbenches: dawn-fde-toolkit + new numero-workbench)
---
...
- **Execution** (`pnpm ce wt:<target>[:<app>] <command>`) — runs any pnpm command from the resolved target's directory. Installed into the workbench by the extension's `on_enable` hook (the extension scaffolds a local CLI; ce itself is not modified). `<target>` resolves to a worktree slug OR the trunk (typically `production/<repo>`).
- **Multi-process** (`pnpm ce wt:pm2 <target>:<app> <cmd> [<target>:<app> <cmd>]...`) — launches each pair as a named pm2 process for parallel dev-server orchestration.
...
| A5 | `pnpm ce wt:<target>[:<app>] <command>` resolves `<target>` to either a worktree slug under `worktrees/` OR the trunk (`production/<repo>`) using the toolkit's existing two-pass scheme. Runs `pnpm <command>` from the resolved directory (or `<resolved>/apps/<app>` when `:<app>` is present). Errors clearly on zero or multiple matches. | vitest integration (workbench fixture with multiple worktrees + a trunk; exercise exact match, suffix match, ambiguous match, zero match, trunk addressing) |
| A6 | `wt:trunk` (and `wt:<wrapped-repo-name>`, e.g. `wt:numero`) is always-present-addressable — no `indusk worktree create` step needed for the trunk. `pnpm ce wt:numero dev` works against the workbench's `production/numero` checkout as soon as the workbench is configured. | vitest integration (workbench fixture with two wrapped repos; assert both trunk forms resolve) |
| A7 | `pnpm ce wt:pm2 <target>[:<app>] <cmd> [<target>[:<app>] <cmd>]...` launches each pair as a named pm2 process visible in `pm2 list` with a name containing the slug. Single invocation, N pm2 processes. | manual smoke (pm2 absent in CI; assert by hand on both workbenches) |
| A8 | `pnpm ce wt:<target> <ce-command>` composes with ce's existing command set — running `pnpm ce wt:cancel-polish dc:up local` brings docker-compose up with the `local` profile against the cancel-polish worktree's `.env.local` (the worktree's env is in scope, not the trunk's). | manual smoke (requires docker; on both workbenches with a known dc:up-shaped command) |
...
| A13 | **The same extension, same CLI, same config schema, and same `pnpm ce wt:` surface work against both `dawn-fde-toolkit` (existing workbench, swapping its scripts for the extension) and a new `numero-workbench` (created as part of this plan, wrapping Numero).** No Avoca-specific assumptions in either workbench's config or CLI behavior. | manual smoke (Sandy runs end-to-end against both workbenches) + vitest integration parameterized with two minimal workbench fixtures, both passing |
...
|----|-----------|-------------------|----------------------|
| U1 | FDEs experience meaningfully less context-switching friction across worktrees after adopting the extension. | UX outcome; only observable through user feedback over weeks of real use. | Sandy reports back after 4 weeks of dogfood use on `numero-workbench`. If "did `pnpm ce wt:<...>` save me time today?" gets yes more often than no, retain; otherwise reshape. Captured as a retrospective audit item. |
