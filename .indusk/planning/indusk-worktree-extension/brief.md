...
**Execution stays in bare `pnpm wt`.** The command-execution surface is `pnpm wt <slug>[:<app>] <command> [args...]` — the exact shape `dawn-fde-toolkit` ships today. The earlier draft proposed `pnpm ce wt:<target> <cmd>`, but pnpm script wildcards lose argument access and ce upstream changes are explicitly out of scope; the bare form has neither problem, is what the toolkit already proves works, and ce composition still works inside it (e.g., `pnpm wt cancel-polish ce dc:up local` cd's into the worktree dir, then runs `pnpm ce dc:up local` from there — composable.env picks up the worktree's `.env.local` because that's the cwd).

**Trunk is implicit.** The toolkit's `wt.sh` already resolves slugs against `worktrees/<slug>` AND `production/<slug>` (two-pass fallback) — trunk and worktrees share one namespace. The indusk version inherits this: `wt trunk` (and `wt <wrapped-repo-name>`, e.g. `wt numero`) is always-present without a create step. The per-project config declares where the trunk lives — typically `production/<repo>` per the workbench shape.
...
**`wt:pm2` is multi-process orchestration, not a flag.** The toolkit's `wt-pm2.sh` takes multiple `<target>:<app> <cmd>` pairs and launches each as a named pm2 process. The indusk surface preserves this: `pnpm wt:pm2 <target>:<app> <cmd> [<target>:<app> <cmd>]...` rather than a `--pm2` flag on the single-target form. (Note: `wt:pm2` is a separate pnpm script — that colon is fine here because it's a top-level script name, not a wildcard.)

**`composeProjectName` enables cross-cwd targeting (composable.env ≥ 1.37.7).** New capability the extension exploits: a workbench's `ce.json` declares a top-level `composeProjectName: "<repo>"` field. With it pinned, docker-compose lifecycle commands (`dc:down`, `dc:restart`, `dc:logs`, etc.) address the named project regardless of cwd — meaning an agent at the workbench root can target a worktree's docker stack without first `cd`-ing into the worktree. The `up`-style command still needs to be invoked from the worktree so ce reads the right env, but lifecycle/inspection is now cwd-free. **Deliberate constraint: only one stack per repo can run at a time** (the name is pinned). This is the right tradeoff for FDE workflows where a single active worktree's stack is the typical case.

**Lifecycle stays in indusk.** `indusk worktree create / refresh / list / preflight` remains the CLI for state operations. Execution (`pnpm wt <slug> <cmd>`) is the user-facing run-anything surface that composes with whatever ce already exposes (dev, build, `ce dc:up local`, etc.).

**Implementation direction (Sandy, 2026-05-20):** the indusk extension installs a **local CLI** into the target project. Concretely: on `indusk extensions enable worktree`, the extension's `on_enable` hook scaffolds the necessary scripts (`scripts/wt.sh` analog or TypeScript equivalent), registers pnpm scripts in the target's `package.json` (`wt`, `wt:pm2`, `preflight`), and seeds `.indusk/worktree-configs/` with a starter config (including the `composeProjectName` recommendation). ce itself is not modified — the bare `pnpm wt <slug> <cmd>` shape works because pnpm already routes script names; no shim or wildcard registration needed. Implementation lives in dusk-the-indusk-project, ships with every project that adopts the extension, and matches the "all code we are working on inside an indusk project" principle.
