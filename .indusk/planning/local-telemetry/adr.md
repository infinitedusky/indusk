...
**We decided for:**
a new **required-by-default** InDusk extension `local-telemetry` (alongside `dash0`, `otel`, etc.) plus a machine-global telemetry daemon running **Jaeger all-in-one + OTel Collector + SQLite log sink as native binaries** (not containerized) spawned and supervised in the admin-UI 1.27.x pattern; binaries distributed via platform-specific npm optional dependencies (`@infinitedusky/telemetry-binaries-darwin-arm64`/`-darwin-x64`/`-linux-arm64`/`-linux-x64`) so that `npm i -g @infinitedusky/indusk-mcp` installs exactly the right binaries for the user's platform with zero custom download logic; extension is auto-enabled by `indusk init` and added by `indusk update` on pre-1.28 projects (the diagnosis loop is available to every InDusk project out of the gate, same way `instrumentation.ts` is auto-scaffolded today); `indusk telemetry start/stop/restart/status` CLI manages the daemon; per-project OTel endpoint env var points services at the Collector's OTLP receiver at `localhost:4318`, the Collector fans out to Jaeger (traces, 16686 UI, REST query API) and the SQLite sink (logs); an MCP tool surface (`get_recent_spans`, `get_trace`, `get_services`, `tail_logs`) gives the agent direct query access.

**And against:**
(a) bundling Jaeger+Collector into the existing `indusk-infra` container (couples telemetry lifecycle to FalkorDB+Graphiti — reindexing code graph bounces the trace buffer); (b) shipping Jaeger+Collector as a separate container alongside `indusk-infra` (forces Docker as a hard prereq for the diagnosis loop — doesn't match the native-process precedent the admin-UI daemon establishes and adds container-orchestration complexity for a loop that's supposed to be a single `start` command); (c) building a custom OTel Collector + SQLite backend without Jaeger (reinvents trace storage + query poorly, skips the free UI); (d) the full Grafana/Tempo/Loki/Prom stack (heavy, dashboards-first, wrong use case — we want "why did this just fail," not "show me p99 latency over 30 days"); (e) SigNoz or Honeycomb self-hosted (even heavier, license complexity, overkill); (f) per-service sidecar OTel exporters instead of a central Collector (duplicates batching/retry/filtering config across every service, blocks future downstream exporters); (g) skipping logs in v1 and surfacing only traces (cripples diagnosis — half of "why did this fail" is log context, not trace shape); (h) opt-in extension (every project should have the diagnosis loop available without a flag — the cost of enabling is ~zero, the cost of a consumer not knowing the loop exists is real).
...
**Accepting:**
a second machine-global daemon on every InDusk user's dev machine (after admin-UI's daemon, this is now two; both auto-start on demand so the user notices only when upgrading); ~100 MB of native binaries per platform (Jaeger ~50 + OTel Collector ~50) installed transparently via npm's platform-matching optional-dependency mechanism — a pattern well-traveled by esbuild/swc/biome/turbo, so zero custom distribution code to maintain; v1 ships the Collector config locked — consumers can't add custom processors until a v2 merge point exists; logs are short-term in a SQLite ring buffer, not long-term searchable (deliberate — long-term observability is Dash0's job); required-by-default means every `indusk init` and `indusk update` does a one-time binary install (user pays the disk cost once per machine).
...
1. **Create a new required-by-default extension at `apps/indusk-mcp/extensions/local-telemetry/`** containing `manifest.json`, `skill.md`, and `.env` template. Manifest declares the skill, the MCP tools provided (`get_recent_spans`, `get_trace`, `get_services`, `tail_logs`), health checks (daemon-reachable, Collector-listening), the daemon-lifecycle hook into `indusk telemetry start`, and a `required: true` flag. `indusk init` auto-enables it (same way it scaffolds `instrumentation.ts`); `indusk update` on pre-1.28 projects adds it as a migration step.
2. **Ship the daemon as native binaries** (Jaeger all-in-one + OTel Collector) spawned and supervised as detached child processes in the admin-UI 1.27.x pattern, not as a container. Binaries distributed via **platform-specific npm optional dependencies** — indusk-mcp's `package.json` lists `@infinitedusky/telemetry-binaries-{darwin-arm64,darwin-x64,linux-arm64,linux-x64}` under `optionalDependencies` with matching `os`/`cpu` constraints, and npm installs only the one matching the consumer's platform at `npm i -g` time. Each platform package bundles the upstream Jaeger + OTel Collector binaries (Apache 2.0 licensed, attribution in the package README). `indusk telemetry start` resolves the binary paths via `require.resolve("@infinitedusky/telemetry-binaries-{platform}/bin/jaeger")` and spawns with `detached: true` + PID tracking in `~/.indusk/telemetry.json`. SQLite log sink runs in-process in otelcol via its file/SQLite exporter.
...
9. **Auto-start on first extension enable; graceful stop when last project disables.** Extension-enable path registers the project in `~/.indusk/telemetry/projects.json` and calls `telemetry start` if daemon isn't running. Extension-disable path deregisters; if registry becomes empty, calls `telemetry stop`. Because the extension is required-by-default, enable-on-`init` is the normal path; explicit `extensions disable local-telemetry` is an escape hatch for users who want to silence it.
10. **Impl Phase 1 = hands-on spike** producing `spike-findings.md` covering: measured query latency vs the 500 ms budget, Jaeger storage-mode choice (in-memory vs Badger), retention default, MCP tool signature ergonomics, **npm platform-package publish + install flow on macOS arm64 + Linux x64** (smallest cross-platform coverage that still proves the pattern), **otelcol variant choice** (full contrib at ~200 MB vs otelcol-core at ~50 MB vs custom-built minimal otelcol via otelcol-builder — the locked pipeline means we only need a small subset of components, so minimal-build may win on size). Spike findings also feed `telemetry-watcher-agent`'s brief.
...
### 7. Ship as a single container image (bundled with indusk-mcp or pulled from a registry)

Package Jaeger + OTel Collector + SQLite log sink in one container image (supervisord or compose); distribute by pulling a versioned tag from a registry or bundling a Dockerfile in the indusk-mcp tarball.

**Rejected because:**
- **Doesn't match the admin-UI daemon precedent.** The same ADR invokes "reuse the admin-UI 1.27.x pattern wholesale," and that pattern is native Node processes (`next start`), not containers. Having two long-lived daemons with different runtime shapes (one native, one containerized) is a consistency tax on the user's mental model for zero architectural gain.
- **Docker prereq for the diagnosis loop.** Forcing every InDusk consumer to have Docker running before they can run `indusk telemetry start` breaks the "dev loop closes in seconds" promise for anyone on a machine without Docker. Admin-UI doesn't require Docker; local-telemetry shouldn't either.
- **Container-distribution custom code.** Pulling a public registry image means CI-to-registry publish pipeline we don't have, or bundling a Dockerfile requires local Docker builds at first-start (slow, fragile). Neither is better than the npm platform-package path, which ships via the same mechanism admin-UI already uses (`npm i -g @infinitedusky/indusk-mcp`).
- **Platform-package distribution is a solved problem.** esbuild, swc, biome, turbo, rollup all ship native binaries through npm's `optionalDependencies` pattern — npm handles platform matching, checksum verification, caching, and offline-after-install automatically. ~zero custom code to maintain.
- **Binary size is smaller than container weight.** Native binaries: ~100 MB per platform (one platform installed per user). Container image: ~210 MB pulled once per machine. Not a huge difference, but the binary path avoids Docker's daemon-always-running overhead.
...
- **Two machine-global daemons now.** After admin-UI (1.27.x) and telemetry (this plan), consumers run two long-lived daemons. Both auto-start, but both are background processes consuming resources. Both native Node-spawned processes — consistent lifecycle shape, consistent supervision code.
- **~100 MB native binaries per platform.** One platform package installed per user via npm optional-deps (the esbuild/swc distribution pattern). Zero custom download logic; npm handles platform matching + checksum + caching.
- **Platform coverage fan-out.** Four platform packages to publish + maintain (darwin-arm64, darwin-x64, linux-arm64, linux-x64). Windows is out of scope for v1. Each upstream Jaeger or OTel Collector version bump requires republishing the four platform packages.
...
| Platform-package publish pipeline is new infrastructure (four packages per upstream bump) | One-time setup: build script that fetches upstream Jaeger + otelcol release artifacts, bundles into each platform package, runs `npm publish` for all four. Spike Phase 1 validates the pipeline on 2 platforms; remaining 2 are mechanical. Version bumps are a known cost, not a surprise. |
| Upstream Jaeger/otelcol version pinning drifts over time | Lock the exact upstream versions in the platform packages' README + build script. Dependabot-style automation (or a manual quarterly bump) keeps them current. Breaking-upstream-change is absorbed at bump time, not at consumer-install time. |
...
| Unsupported platform (Windows, BSD, musl-linux) | Out of scope for v1 — npm's `os`/`cpu` fields naturally exclude them from optional-deps install. Clear error message at `indusk telemetry start`: "local-telemetry requires darwin/linux arm64/x64; see docs to request platform support." Expansion is additive (publish another platform package). |
| Required-by-default surprises consumers who expected opt-in | `indusk init` prints that it's enabling local-telemetry; `indusk extensions disable local-telemetry` is the explicit escape hatch; docs call out the escape hatch in the migration notes. Same pattern admin-UI daemon uses (auto-start on `indusk ui`; explicit `stop` to suppress). |
...
- **Architecture Mermaid in `overview.md`:** sequence diagram showing `indusk telemetry start` → otelcol + Jaeger processes spawned → service emits OTLP to `localhost:4318` → Collector receives → fans to Jaeger (traces) + SQLite (logs) → agent calls MCP tool → daemon query → JSON response. Parallel to admin-UI's overview diagram.
