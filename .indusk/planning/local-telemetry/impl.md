...
| T18 | `indusk init --extensions local-telemetry` on a fresh project produces a working setup: `.env` written, project registered, MCP tools in `.mcp.json`, daemon auto-starts, a test script can emit + query spans — zero further manual config. | Phase 0 | Phase 6 | passing |
| T19 | `indusk extensions disable local-telemetry` deregisters the project, removes MCP tools from `.mcp.json`, and stops the daemon iff the registry becomes empty. No orphan processes, no stale MCP entries. | Phase 0 | Phase 6 | passing |
| T20 | An existing project (already using `dash0`, already has `instrumentation.ts`) can opt into local-telemetry via `indusk extensions enable local-telemetry` without rewriting `instrumentation.ts` — env file swap is the only behavioral change. | Phase 0 | Phase 6 | passing |
...
| T23 | A project with `disabled_extensions: ["local-telemetry"]` in `.indusk/config.json` opts out: `indusk init` does NOT auto-enable, `indusk update` does NOT migrate, no daemon registration, no `.mcp.json` mutation. Escape-hatch contract for security/perf-constrained projects. | Phase 0 | Phase 6 | passing |
...
- [x] jaeger_mcp extension enabled in `packages/telemetry-binaries-shared/jaeger-config.yaml` (on `mcpPort`, default 16687) and mirrored in daemon.ts's inline-rendered config. `mcpPort` is surfaced as a field on `DaemonMeta` and persisted to `~/.indusk/telemetry.json`. Allocation uses `findFreePort(0)` (OS-assigned) to avoid TIME_WAIT cross-test collisions, not pinned 16687.
- [x] Extension on_enable writes/refreshes the project's `.mcp.json` via `upsertJaegerEntry(projectPath, mcpPort)` in `apps/indusk-mcp/src/bin/commands/telemetry.ts` — entry shape `{ type: "http", url: "http://localhost:{mcpPort}/mcp" }` per dash0 pattern. On_disable removes it via `removeJaegerEntry`. Every daemon `start` refreshes all registered projects' entries with the current `mcpPort`.
- [x] Custom MCP tool landed at `apps/indusk-mcp/src/tools/telemetry-tools.ts` (file-location variant of the planned `src/server/tools/telemetry/tail-logs.ts` — same shape, simpler path). `tail_logs` with zod inputs (`service?`, `level`, `since_minutes`, `limit` with 50/200 defaults + caps). OTLP-envelope JSONL parser normalizes `resourceLogs → scopeLogs → logRecords` into flat entries. Response includes `truncated`, `count`, `window_actual`, `hints`.
- [x] `registerTelemetryTools(server)` wired in `apps/indusk-mcp/src/server/index.ts`. Tool availability gated on `isTelemetryActive()` (project is in the registry AND daemon is running).
- [x] Extended `apps/indusk-mcp/src/bin/commands/telemetry.ts` with `telemetryTail` / `telemetryTrace` / `telemetryServices` / `telemetryReset`. `tail` reuses the JSONL parser path (normalized to a human-readable one-line-per-record format). `trace` hits `/api/traces/<id>`, `services` hits `/api/services` — both via Jaeger REST. `reset` stops → truncates `~/.indusk/telemetry/logs.jsonl` → restarts.
- [x] Subcommands wired in `cli.ts` (commander@13 pattern — options on parent command, subcommands read via `optsWithGlobals()` where needed, e.g., `reset` inherits `--otlp-port` / `--ui-port`).
- [x] Queued follow-up plan entry in `master.md` for `unified-telemetry-query` (the "same interface for Jaeger + Dash0" translation layer). Linked from CLAUDE.md and the overview page's Future Work section. Brief-draft deferred until 1.28.x has one real dogfood session of jaeger_mcp direct use to surface the bumbling patterns the wrapper needs to structurally prevent.
...
#### Phase 5 Context
- [x] Appended to CLAUDE.md Known Gotchas: the jaeger_mcp direct-wiring pattern (mcpPort auto-assigned + `.mcp.json` upsert on register/refresh on every daemon start), Jaeger's 8 pre-computed tools replacing our planned custom wrappers, the separation of trace-side (jaeger_mcp) vs log-side (custom `tail_logs`) responsibilities, and the forward-pointer to the unified-telemetry-query plan that'll collapse this split in a future release.

#### Phase 5 Document
- [x] Extended `apps/indusk-docs/src/reference/telemetry/cli.md` with a `## Query subcommands` section documenting `tail` / `trace` / `services` / `reset` — purpose, flags, sample output, failure modes. Table at top maps each to its backend (otelcol file sink vs Jaeger REST vs daemon lifecycle) so readers can pick the right command at a glance. Existing lifecycle-only quick-reference points at it.
- [x] Drafted `apps/indusk-docs/src/reference/telemetry/overview.md` — architectural orientation page with Mermaid sequence diagram of the daemon model, platform-packages explanation (esbuild/swc pattern, ~282 MB per platform, `createRequire` resolution), extension wiring (enable → register → `.mcp.json` upsert; disable → deregister → daemon-stops-iff-empty), agent vs human surface split (8 jaeger_mcp tools for traces, custom `tail_logs` for logs, CLI mirror), environment routing table, migration-from-Dash0-only flow, known gotchas, future-work forward-pointers to telemetry-watcher-agent + unified-telemetry-query. Numbers from spike-findings.md (p95=12.2ms, 100k span cap, 50MB log sink). Cross-linked from cli.md See also.
...
- [x] `init.ts` auto-enable is free: existing `autoEnableExtensions(projectRoot)` call at step 11 now discovers required extensions in its new Pass 1 (`if (!ext.required) continue;` guard + `extensionsEnable` + `(required-by-default)` suffix log). T18 verifies stdout mentions `local-telemetry`.
- [x] `update.ts` migration step landed as new "[Required Extensions]" section after third-party extensions + before git-ignores (line 466-473 of `update.ts`): `await autoEnableExtensions(projectRoot)` — same codepath as init, so local-telemetry's required-by-default propagates automatically. Idempotent on re-run (isEnabled check in Pass 1 short-circuits). Also honors `INDUSK_SKIP_SELF_UPDATE=1` now so offline / deterministic contexts don't hit npm.
- [x] Required-resolution refactor: `autoEnableExtensions` split into Pass 1 (required-by-default with `disabled_extensions` escape-hatch consulted via new `isExtensionExplicitlyDisabled(projectRoot, name)` helper in `lib/config.ts`) + Pass 2 (original detection-based flow). `ExtensionManifest.required?: boolean` + `hooks.on_enable/on_disable?: string` added to the manifest type. `extensionsDisable` now fires `on_disable` BEFORE renaming — mirror of how `extensionsEnable` fires `on_init`/`on_enable`. `runHook` gains `INDUSK_BIN` env-var override so hooks can target a specific CLI (test-critical; also useful for preview users).
- [x] Migration guidance in `overview.md` already covers this shape — "Migration from Dash0-only" section enumerates the 6 steps (detect, write `.env`, register, upsert `.mcp.json`, leave `instrumentation.ts`, leave `dash0`) with rollback note. No further edit needed; Phase 6 Document gate confirms it's readable + concrete.
- [x] dash0 + local-telemetry coexistence: T20 asserts both extensions remain enabled after the migration (`.indusk/extensions/dash0/manifest.json` + `.indusk/extensions/local-telemetry/manifest.json` both present); `instrumentation.ts` SHA unchanged; composable.env profile selection (in the .env merge order) drives the runtime endpoint. Overview page's env-routing table documents the per-profile mapping.
- [x] Both install paths validated via T18 (fresh init) + T20 (existing upgrade) + T23 (escape hatch). T19's two-project case verifies the daemon-stops-iff-empty contract. Registry path normalization (`normalizeProjectPath` via `realpathSync`) added to `registry.ts` so `$(pwd)` hook calls + absolute-path programmatic calls produce identical registry keys — was a real test-surfaced bug that'd have hit macOS users (`/var` ↔ `/private/var`).

#### Phase 6 Verification
- [x] T18 authored red at `apps/indusk-mcp/src/__tests__/telemetry-init-fresh.test.ts` — `mkdtemp` project + `indusk init --no-index` in a tmp `INDUSK_HOME` → asserts manifest at `.indusk/extensions/local-telemetry/`, stdout mentions the auto-enable, registry updated, `.mcp.json` jaeger entry wired, daemon running. Red today because `autoEnableExtensions` doesn't consult `required: true`.
- [x] T19 authored red at `telemetry-extension-disable.test.ts` — exercises the full user CLI chain `indusk extensions disable local-telemetry`, asserting extension moves to disabled, registry empties, `.mcp.json` jaeger entry is removed, daemon stops iff empty. Two-project case verifies the daemon stays up when only one project disables. Red today because `extensionsDisable` doesn't fire `on_disable`.
- [x] T20 authored red at `telemetry-existing-project-upgrade.test.ts` — pre-1.28-shaped project with `dash0` enabled + `instrumentation.ts` at root → `indusk update` → asserts `local-telemetry` is added as a migration step while `instrumentation.ts` SHA stays unchanged, dash0 stays enabled, registry + `.mcp.json` wired, and a second update is a no-op. Red today because `update.ts` never touches extensions.
- [x] T23 authored red at `telemetry-explicit-disable.test.ts` — escape-hatch contract: project with `disabled_extensions: ["local-telemetry"]` in `.indusk/config.json` → `indusk init` does NOT enable, register, wire `.mcp.json`, or start the daemon, AND stdout visibly surfaces the skip. Red today because `disabled_extensions` isn't consulted anywhere.

#### Phase 6 Context
- [x] Appended five new CLAUDE.md Known Gotchas entries covering the Phase 6 surface: (a) required-by-default extension resolution via autoEnableExtensions two-pass split + `disabled_extensions` escape hatch, (b) `extensionsDisable` fires `on_disable` before rename (mirror of enable/on_init, previously silently broken), (c) `INDUSK_BIN` env-var hook override pattern for preview/test scenarios, (d) telemetry registry realpath normalization (macOS `/var` ↔ `/private/var`), (e) `update.ts` step 7b migration path + `INDUSK_SKIP_SELF_UPDATE` honoring. Each entry is load-bearing — all were pre-existing silent-bug classes surfaced by Phase 6 tests.

#### Phase 6 Document
- [x] Extended overview.md's Extension Wiring section with the `disabled_extensions` escape-hatch shape: exact `.indusk/config.json` snippet, concrete stdout from `init` under the escape, behavior of `update` on an opted-out project, how to opt back in, and the deliberate absence of a CLI affordance (hand-edit only). The pre-existing Migration-from-Dash0-only section already covers the 6-step upgrade flow (dash0 extension coexists, `instrumentation.ts` unchanged, env component merge, register + `.mcp.json` upsert, rollback path) — audited and left as-is.
