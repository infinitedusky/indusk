...
| T8 | When `<repo>-workbench` exists but is NOT a completed InDusk workbench (no `.indusk/config.json`), `setup` reports a distinct "exists but is not an InDusk workbench" error and does NOT advise `indusk update` | Phase 0 | Phase 2 | passing |
| T9 | A `setup` run that fails during `init` (after the workbench dir is created) leaves no `<repo>-workbench` directory behind — setup is atomic | Phase 0 | Phase 2 | passing |
...
- [x] Refine the collision guard in `setup.ts`: after `existsSync(workbenchDir)`, branch on `existsSync(join(workbenchDir, ".indusk", "config.json"))`. Real workbench → "a workbench already exists … run `indusk update`". Dir without config → distinct "`<dir>` exists but is not an InDusk workbench — Remove it and re-run" (no `indusk update`).
- [x] Make `setup` atomic: post-guard `writeFileSync` + `await init(...)` wrapped in `try/catch`; on failure `rmSync(workbenchDir, { recursive: true, force: true })` + clean error + exit 1. Safe: collision guard proved the dir didn't pre-exist; `rmSync` removes the trunk symlink as a link (no recursion into `../<repo>`), so the wrapped repo is untouched.

#### Phase 2 Verification
- [x] T8: empty `<repo>-workbench` (no config) → stderr no longer matches `/indusk update/`, now matches `/not an InDusk workbench/i`. T5 (real-workbench collision → "indusk update") stays green — the two diverge on config presence. `pnpm --filter @infinitedusky/indusk-mcp exec vitest run src/__tests__/setup-command.test.ts` → 9/9.
