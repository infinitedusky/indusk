...
#### Phase 8 Context
- [ ] Add to CLAUDE.md Known Gotchas: "Admin UI detects the falsification phase by title substring (case-insensitive `"Falsification"` in the phase title). The convention `### Phase N: Falsification — {summary}` from the `/falsify` skill is grep-friendly; if a plan's falsification phase uses a different title, it won't be recognized and will render as a regular phase. The validator doesn't enforce the naming — future polish if drift becomes a problem." Add to Conventions: "Admin UI renders impl phases in three positional groups: pre-falsification phases (the main `Phases` section), the falsification phase itself (rendered as the `Falsification` section with trajectory rows = hypotheses and checklist items = fix items), and post-falsification phases (the `Follow-up Phases` section — fix-in-scope derivatives of the ritual)."

#### Phase 8 Document
- [ ] Update `apps/indusk-docs/src/reference/admin-ui/overview.md` with a new subsection under "What each page shows" describing the falsification-phase rendering: how hypotheses are sourced from trajectory rows (`Passes at: {falsification-phase-number}`), how fix items come from the phase's checklist, and how follow-up phases appear in their own section. Reference the falsify skill's naming convention. Include a short "Legacy plans" callout noting that plans authored pre-1.27.4 with `falsification.md` files still render via the log-based path.
