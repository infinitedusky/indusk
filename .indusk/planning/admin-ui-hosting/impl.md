...
| T22 | `daemonStatus()` returns `running: false` when the recorded PID points at a live process that is NOT listening on the recorded port (PID-reuse simulation). Today `isAlive`-only check false-positives and `uiStart` refuses to spawn a second daemon, blaming a stranger's PID. | Phase 0 | Phase 7 | passing |
| T23 | `daemonStop()` does NOT signal a process whose identity can't be confirmed — if the PID is alive but the recorded port is not listening, treat it as stale and clean up without SIGTERM. A spawned noop child sitting at the recorded PID must survive `daemonStop`. | Phase 0 | Phase 7 | passing |
| T24 | `readRegistry()` called against a malformed `projects.json` must quarantine the file (rename to `projects.json.corrupt.{ISO}.bak`) before `addProject()` writes a fresh registry — the malformed bytes must be preserved, not silently overwritten by the next write path. | Phase 0 | Phase 7 | passing |
| T25 | `resolveOpenPath()` returns `/p/{name}/` when cwd is a subdirectory of a registered project's root (e.g., `{root}/apps/indusk-mcp`), not just when cwd is the root itself — otherwise T16's cwd-aware promise regresses as soon as the user runs `indusk ui` from anywhere but the project root. | Phase 0 | Phase 7 | passing |
| T26 | Admin UI's `PlanDetail` renders the "Falsification" section using the phase whose title contains "Falsification" (case-insensitive) — trajectory rows + impl items — for plans using the new phase-authoring flow. That phase is OMITTED from the regular `Phases` section to avoid double-rendering. | Phase 0 | Phase 8 | passing |
| T27 | Phases authored AFTER the falsification phase (fix-in-scope follow-ups derived from the ritual) render as a distinct "Follow-up Phases" section below the falsification section — not mixed in with the pre-falsification phases above. | Phase 0 | Phase 8 | passing |
| T28 | Legacy plans with a completed `falsification.md` file (pre-1.27.4) still render the falsification section from the log file via `readFalsificationLog`; no regression when no falsification phase is present in impl.md. | Phase 0 | Phase 8 | passing |
| T29 | `<CollapsibleSection>` accepts an optional `persistKey` prop; when set, the component reads its initial open/closed state from `localStorage[persistKey]` (falling back to `defaultOpen` when no stored value) and writes the state back on every toggle. Persistence is per-section, keyed uniquely per (plan, section), so users don't re-open/re-close sections on every page visit. | Phase 0 | Phase 9 | passing |
| T30 | Every `<CollapsibleSection>` rendered in `PlanDetail` and in `app/p/[project]/layout.tsx` (sidebar Research group, if present) passes a stable `persistKey` derived from the plan name + section slug. Across page reloads, the rendered HTML reflects the stored state (i.e., previously-closed sections stay closed after reload). | Phase 0 | Phase 9 | passing |
...
### Phase 7: Falsification — daemon identity + registry quarantine + cwd walk-up

**Goal**: close four specific gaps found by investigating the daemon, registry, and cwd-resolution code against the plan's attested state. Two are PID-reuse hazards (false-positive "already running" and SIGTERM-to-unrelated-process when the OS recycles a crashed daemon's PID), one is a silent-data-loss path (a hand-edit that breaks `projects.json` JSON causes `readRegistry` → `emptyRegistry()` → `addProject` to clobber the corrupt file with a fresh registry, destroying all prior entries), and one is a T16 regression (`resolveOpenPath` only matches when cwd equals the project's registered root exactly — running `indusk ui` from any subdirectory silently falls through to `/`). Each trajectory row below pins one hypothesis; each impl item below is the concrete fix.

- [x] Add `verifyIdentity(pid: number, port: number): Promise<boolean>` helper to `apps/indusk-mcp/src/lib/admin/daemon.ts` that returns true only when BOTH `isAlive(pid)` AND `isPortListening(port)` resolve true. The port check is the load-bearing discriminator — a reused PID pointing at some unrelated process (bash, postgres, another vitest) won't be listening on the daemon's recorded port. Keep the existing `isAlive`/`isPortListening` exports; the new helper composes them.
- [x] Rewire `daemonStatus()` to use `verifyIdentity(pid, meta.port)` in place of the current bare `isAlive(pid)` check. On identity-verification failure, clean up the PID + meta files (they're stale) and return `{ running: false }`. The port-listening probe adds ~500ms worst-case to status calls — acceptable for a CLI interaction, and T22's test writes red until this is in place.
- [x] Rewire `daemonStop()` to gate its SIGTERM on `verifyIdentity(pid, meta.port)`. If PID alive but port not listening, treat as stale: do NOT SIGTERM (avoids killing a stranger's process that inherited the PID), clean up PID + meta files, return `stopped: true, signaledPid: pid, usedSigkill: false`. If identity verified, proceed with the existing SIGTERM → 3s poll → SIGKILL fallback.
- [x] Quarantine malformed `projects.json` in `apps/indusk-mcp/src/lib/admin/registry.ts`'s `readRegistry()`: when JSON.parse throws OR shape-validation fails, rename the existing file to `projects.json.corrupt.{ISO-timestamp}.bak` via `renameSync` before returning `emptyRegistry()`. Print a single-line warning to `process.stderr` naming the quarantined path. Any subsequent `writeRegistry` creates a brand-new `projects.json`; the malformed bytes are preserved on disk for user recovery. The quarantine path includes the ISO timestamp (not just `.bak`) so repeated corruption events don't overwrite each other.
- [x] Extend `resolveOpenPath()` in `apps/indusk-mcp/src/bin/commands/ui.ts` to walk up cwd's parents until it finds a registered project match. For each ancestor (starting at cwd, ending at filesystem root), compare its `realpathSync` against each registered project's `realpathSync`'d `path`. First match wins; return `/p/{name}/`. Cap at 40 `dirname` iterations as a loop sentinel (filesystem paths never exceed this in practice). Fall through to `/` only when the walk-up completes without a hit.
- [x] **Discovered during Phase 7 regression check**: fix pre-existing test-mock omission in `apps/indusk-admin/src/app/p/[project]/page.test.tsx` — the `@/lib/planning-reader` mock doesn't include `readProjectResearch` (added to the layout in Phase 6 item 7), causing the test to fail at module-load time with "does not provide an export named 'readProjectResearch'". Add `readProjectResearch: async () => []` to the mock. Not caused by Phase 7, but Phase 6's regression check relied on `tsc --noEmit` (which doesn't load the test) rather than `pnpm vitest run`, so it slipped through.
- [x] Bump `apps/indusk-mcp/package.json` version → 1.27.5 (patch-level hardening). Add a 1.27.5 entry to `apps/indusk-docs/src/changelog.md` naming the four fixes and their rationale: "daemon-identity verification (PID-reuse hardening), registry quarantine on malformed JSON (data-loss prevention), cwd-walk-up in bare `indusk ui` (T16 subdirectory regression fix)".

#### Phase 7 Verification
- [x] T22 (write red): committed `apps/indusk-mcp/src/lib/admin/__tests__/daemon-identity.test.ts` with the "PID reuse makes status report false-positive running" case. Setup: `INDUSK_HOME=<tmp>`, write `admin-ui.pid` containing `process.pid` + `admin-ui.json` with `port: 65001`. Asserts `await daemonStatus()` returns `{ running: false }`. Red at authoring, green after `verifyIdentity` rewire.
- [x] T23 (write red): in the same test file, "daemonStop does not SIGTERM a stranger" — spawns detached noop child, writes pidfile pointing at it with port 65001, calls `daemonStop`, asserts `isAlive(childPid)` after 200ms. Red at authoring, green after identity gate in `daemonStop`. Both daemon-identity assertions: `pnpm vitest run src/lib/admin/__tests__/daemon-identity.test.ts` → 2/2 passing.
- [x] T24 (write red): committed `apps/indusk-mcp/src/lib/admin/__tests__/registry-quarantine.test.ts` with two cases — (a) malformed JSON quarantined to `.corrupt.{ISO}.bak`, fresh registry holds new entry, backup contents preserved byte-for-byte; (b) wrong-shape registry (array instead of object) also quarantined, idempotent across repeated reads. Red at authoring, green after `readRegistry` rewire. `pnpm vitest run src/lib/admin/__tests__/registry-quarantine.test.ts` → 2/2 passing.
- [x] T25 (write red): committed `apps/indusk-mcp/src/__tests__/resolve-open-path-walkup.test.ts` with 4 cases — deep subdirectory, one-level below, cwd-at-root (regression guard), and cwd-outside-any-project. Exported `resolveOpenPath` from `ui.ts` (minimal surface addition acknowledged in the impl item). 2 walk-up cases red at authoring, green after walk-up; 2 regression guards green throughout. `pnpm vitest run src/__tests__/resolve-open-path-walkup.test.ts` → 4/4 passing.
- [x] Regression check: admin-ui-hosting–touched suites all green. Indusk-mcp admin tests: `pnpm vitest run src/lib/admin src/__tests__/resolve-open-path-walkup.test.ts src/__tests__/admin-bundle-pack.test.ts src/__tests__/cli-bare-ui-cwd-aware.test.ts` → 20/20 passing. Admin-ui app: `pnpm --filter indusk-admin vitest run` → 91/91 passing (15 files). Pre-existing failures in `falsification/integration.test.ts` (5 cases asserting old `/falsify` skill content — rewritten for phase-authoring in 1.27.4) and `plan-parser.test.ts` (1 case asserting `agent-roles` still active — archived earlier) are NOT caused by Phase 7; they predate this branch and are tracked as separate test-fixture stale-content issues.

#### Phase 7 Context
- [x] Added three new entries to CLAUDE.md Known Gotchas: (1) daemon identity requires PID liveness + port listening (PID-reuse hardening); (2) `readRegistry` quarantines malformed `projects.json` to `.corrupt.{ISO}.bak`; (3) bare `indusk ui` walks up cwd's parents (40-iteration cap) to find the nearest registered project. All three entries land immediately below the existing commander@13 gotcha in the admin-ui Known Gotchas cluster.

#### Phase 7 Document
- [x] Added three recovery-recipe entries to `apps/indusk-docs/src/reference/admin-ui/cli.md` under the existing "Recovery recipes" section: (a) "Daemon reports not running right after a crash (1.27.5+)" — explains the identity gate + auto-sweep of PID/meta files, points at `admin-ui.log` for crash cause. (b) "Registered projects seem to have disappeared (1.27.5+)" — explains the quarantine rename, includes the four-step recovery command block with `cat` → hand-fix → `mv` → `indusk ui status`. (c) "Bare `indusk ui` from a subdirectory of my project (1.27.5+)" — documents the walk-up, the 40-ancestor cap, and the diagnostic `jq` command to verify registration. All three reference concrete file paths the user can inspect.

### Phase 8: Admin UI falsification-aware rendering (fix-in-scope from Phase 7 dogfood)

**Goal**: the `/falsify admin-ui-hosting` dogfood surfaced that the admin UI's `FalsificationSection` only knows about the legacy `falsification.md` file — so plans using the new phase-authoring flow render "No falsification ritual run for this plan" even when they have a Falsification Phase in impl.md. This phase teaches the admin UI to recognize the new shape: the falsification phase shows as the results section (hypotheses = trajectory rows, fix items = impl checklist), and any phases AFTER it render as "Follow-up Phases". Legacy plans keep working — backcompat preserved. This phase itself dogfoods its own rendering: admin-ui-hosting's impl now has Phases 1–6 (regular) → Phase 7 (Falsification) → Phase 8 (follow-up).

- [x] Add `splitPhasesAroundFalsification(phases: Phase[]): { pre: Phase[]; falsification: Phase | null; post: Phase[] }` helper to `apps/indusk-admin/src/lib/phases.ts`. Detects the falsification phase by title containing `"Falsification"` (case-insensitive substring). If multiple match, uses the first. If none match, returns `{ pre: phases, falsification: null, post: [] }`. Phases before the falsification phase become `pre`; phases after become `post`. Pure, no side effects.
- [x] Extend `apps/indusk-admin/src/components/PlanDetail.tsx` to call `splitPhasesAroundFalsification` (via a new `ImplSections` wrapper that replaces the bare `PhasesSection` call). Render three sibling sections: `PhasesSection` over `pre` (testid `phases-section`), `FalsificationSection` with the detected phase, and a second `PhasesSection` over `post` (testid `followup-phases-section`). Generalized `PhasesSection` to accept `{ phases, heading, testId }` props so both pre and post can reuse the same component — no separate `FollowUpPhasesSection` needed.
- [x] Enhance `FalsificationSection` in `PlanDetail.tsx`: accepts `phase: Phase | null` prop. When `phase` is non-null, delegates to a new `FalsificationPhaseSection` component that renders trajectory rows as a Hypotheses table (ID / Asserts / State) and checklist items as a Fix items list. Status badge reads `complete` when all rows are `passing`/`skipped` AND no unchecked items remain. When `phase` is null AND legacy `plan.falsification` is set, keeps the existing log-based rendering. When both null, keeps the empty-state message.
- [x] Add `extractChecklistItems(markdown: string): Array<{ text: string; checked: boolean }>` helper in `phases.ts`. Regex-based, matches `- [ ]` / `- [x]` at line start with mandatory space between brackets. Uppercase `X` and missing-space shapes rejected (stays aligned with standard GFM task-list syntax).
- [x] Unit tests for phases.ts at `apps/indusk-admin/src/lib/phases.test.ts` — 8 tests: 5 for `splitPhasesAroundFalsification` (middle, none, end, case-insensitive, multiple-first-wins) + 3 for `extractChecklistItems` (mixed checked/unchecked, empty, malformed rejected). All green.
- [x] Bump `apps/indusk-mcp/package.json` version → 1.27.6. Add changelog entry under "Fixed" for 1.27.6 naming the admin-UI falsification-phase rendering fix, noting the three sub-changes (split phases, falsification-phase rendering, follow-up phases), and naming the backcompat preservation for legacy `falsification.md` plans.

#### Phase 8 Verification
- [x] T26 (write red): committed `apps/indusk-admin/src/lib/phases.test.ts` with 8 unit tests: 5 for `splitPhasesAroundFalsification` (middle-of-list, no-falsification, last-phase, case-insensitive, multiple-first-match) and 3 for `extractChecklistItems` (mixed checked/unchecked, empty, malformed rejected). Red at authoring (8/8 fail — helpers don't exist), green after implementation. `pnpm vitest run src/lib/phases.test.ts` → 8/8 passing.
- [x] T27 (write red): extended `apps/indusk-admin/src/components/PlanDetail.test.tsx` with four browser-mode component tests using a `mockMixedPhasePlan` fixture — Phase 1 (regular) + Phase 2 (Falsification) + Phase 3 (follow-up). Assertions: phases-section contains Phase 1 only (not Phase 2); falsification-section contains T2 hypothesis + fix items from Phase 2's checklist; followup-phases-section contains Phase 3; followup-phases-section absent when no post-falsification phases exist.
- [x] T28 (write red): added the legacy-log backcompat component test to the same file — plan with no falsification phase in impl but `plan.falsification` populated with a hypothesis + terminator entry. Asserts the log-based rendering works (hypothesis-fix-in-scope item + falsification-terminator present, falsification-fix-items absent — confirming the phase path did NOT fire).
- [x] All Phase 1–7 tests still green. `cd apps/indusk-admin && pnpm vitest run` → 104/104 passing across 16 files. Pre-existing indusk-mcp failures (falsification/integration + plan-parser, unrelated to admin-ui) predate Phase 8 and remain tracked separately.

#### Phase 8 Context
- [x] Added two CLAUDE.md Known Gotchas entries immediately after the 1.27.5 hardening entries: (1) Admin UI detects the falsification phase by case-insensitive title substring via `splitPhasesAroundFalsification()`; title drift from the skill's recommended convention risks non-detection but the validator does not enforce naming. (2) `PlanDetail` renders impl phases in three positional groups (pre / falsification / post) with distinct testids, and the legacy `falsification.md` rendering path is preserved as a mutually-exclusive alternative when no falsification phase is detected in impl.md.

#### Phase 8 Document
- [x] Updated `apps/indusk-docs/src/reference/admin-ui/overview.md`: (1) existing Phases table row now notes the falsification phase is EXCLUDED from this section; (2) new Falsification (1.27.6+ phase path) table row describes the Hypotheses table + Fix items list + status-badge logic; (3) renamed legacy row to "Falsification (legacy)" with "used only when impl has no falsification phase" guard language; (4) new "Follow-up Phases (1.27.6+)" table row; (5) new standalone paragraph under the table titled "Falsification rendering (1.27.6+)" explaining the detection heuristic (first phase title containing "Falsification", case-insensitive), the hoist-out behavior, and the legacy-backcompat story. The user-facing docs now accurately describe all three render paths.

### Phase 9: CollapsibleSection state persistence (fix-in-scope polish)

**Goal**: every `<CollapsibleSection>` in the admin UI remembers whether it was open or closed across page reloads. Surfaced by the user mid-dogfood: the Brief section re-opens on every visit even after the user explicitly closed it on the previous page view, which becomes annoying fast for plans with a long brief. Fix: persist per-section state in `localStorage`, keyed by a caller-supplied `persistKey` so the mapping is stable across renders. Falls back to the current `defaultOpen` behavior when no key is supplied.

- [x] Extended `apps/indusk-admin/src/components/ui/CollapsibleSection.tsx` with optional `persistKey?: string` prop. Reads initial state from `localStorage[persistKey]` (values `"1"` / `"0"`) with `defaultOpen` fallback. Toggle writes back. Guards `typeof window !== "undefined"` for SSR and swallows localStorage exceptions (privacy mode / quota) via try/catch.
- [x] Component was already a client component (`"use client"` from prior work) — no conversion needed. Initial state read happens inside `useState(() => readPersisted(...))`, which runs on first render on the client; server renders `defaultOpen` and hydration reconciles (acceptable tradeoff documented in CLAUDE.md).
- [x] Wired `persistKey` into every CollapsibleSection in `PlanDetail.tsx`: Research (`plan:{name}:section:research`), Brief (`plan:{name}:section:brief`), Test Plan (`plan:{name}:section:test-plan`), ADR (`plan:{name}:section:adr`), and every phase (`plan:{name}:phase:{N}`). `BriefSection` and `PhasesSection` now take `planName` as a prop; `ImplSections` passes `plan.name` through. RawDocumentsSection (malformed-plan diagnostic mode) left unchanged — per-session diagnostic work doesn't benefit from cross-session memory.
- [x] Skipped per-project-layout CollapsibleSection wiring — the sidebar Research group in `app/p/[project]/layout.tsx` is NOT actually a CollapsibleSection (it's a plain `<nav>` block with a conditional render), so there's no state to persist there. Verified by grep; nothing to change.
- [x] Bumped `apps/indusk-mcp/package.json` version → 1.27.7. Added changelog entry under "Added" naming the persistKey prop, the convention `plan:{planName}:section:{slug}`, the SSR/privacy-mode safe fallback, and the user's original motivation (Brief re-opening on every page visit).

#### Phase 9 Verification
- [x] T29 (write red): committed `apps/indusk-admin/src/components/ui/CollapsibleSection.test.tsx` with 4 browser-mode cases — absent-persistKey fallback, stored "0" overrides `defaultOpen: true`, stored "1" overrides `defaultOpen: false`, and toggle round-trip writes "1"/"0" to localStorage. All 4 passing.
- [x] T30 (write red): added two tests in `PlanDetail.test.tsx` under "PlanDetail — collapsible section state persists (T30, Phase 9)" — pre-seed `plan:alpha-feature:section:brief = "0"` asserts Brief starts closed (persisted state wins over in-code `defaultOpen: true`), and the empty-localStorage case asserts Brief starts open (regression guard for the first-visit fallback). Used pre-seeding pattern instead of click-then-rerender because `unmount()` + `render()` in vitest-browser-react had ordering interference with downstream tests; pre-seeding is the cleanest isolation shape and tests the end-to-end contract that matters to the user.
- [x] All Phase 1–8 tests still green. `cd apps/indusk-admin && pnpm vitest run` → 110/110 passing across 17 files. Added `beforeEach(() => localStorage.clear())` at the top of `PlanDetail.test.tsx` to prevent persisted state from one test leaking into another — surfaced when the T30 first pass (click-then-rerender) was failing later tests due to stale localStorage keys.

#### Phase 9 Context
- [ ] Add to CLAUDE.md Known Gotchas: "`<CollapsibleSection>` persists open/closed state per-section via localStorage when `persistKey` is supplied. Keys should be stable and unique — convention is `plan:{planName}:section:{slug}` or `project:{projectName}:section:{slug}`. Without `persistKey`, the component behaves as before (ephemeral state, `defaultOpen` on every render). The component is a client component; SSR renders `defaultOpen` and the post-hydration effect reconciles to the persisted value, producing a brief flash if `persisted ≠ defaultOpen` — acceptable tradeoff for the memoryful UX."
