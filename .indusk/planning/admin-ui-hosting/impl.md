...
#### Phase 7 Context
- [ ] Add to CLAUDE.md Known Gotchas (dusk project-level): "Daemon identity verification requires BOTH PID-liveness AND port-listening — bare `kill(pid, 0)` alone false-positives after crashes when the OS recycles the PID. `daemonStatus` and `daemonStop` both gate on the combined check; a mismatch triggers quarantine cleanup, not a signal." Add: "`readRegistry` quarantines malformed `projects.json` to `projects.json.corrupt.{ISO}.bak` rather than silently returning empty — if registered projects seem to have vanished, look in `~/.indusk/` for a `.corrupt.*.bak` file with the pre-damage contents." Add to Conventions (admin-ui behavior): "`indusk ui` from any subdirectory of a registered project opens `/p/{name}/`; the CLI walks up cwd's parents to find the nearest registered root (capped at 40 ancestors), falling through to `/` only when no match is found."

#### Phase 7 Document
- [ ] Update `apps/indusk-docs/src/reference/admin-ui/cli.md` with a new "Troubleshooting" subsection: (a) "Daemon reports not running after a crash" — explain the identity gate, mention that stale PID + meta files are auto-cleaned on next `ui status` / `ui stop`, reference the `admin-ui.log` for the underlying crash. (b) "Registered projects seem to have disappeared" — explain the quarantine rename (`projects.json.corrupt.{ISO}.bak`), show the recovery command (`mv ~/.indusk/projects.json.corrupt.{ISO}.bak ~/.indusk/projects.json` after fixing the JSON by hand). (c) "`indusk ui` from subdirectory" — document that the CLI walks up cwd to find the nearest registered root, so deep invocations still land on the expected project page.
