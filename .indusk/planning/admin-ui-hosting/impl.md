...
| T26 | Admin UI's `PlanDetail` renders the "Falsification" section using the phase whose title contains "Falsification" (case-insensitive) — trajectory rows + impl items — for plans using the new phase-authoring flow. That phase is OMITTED from the regular `Phases` section to avoid double-rendering. | Phase 0 | Phase 8 | planned |
| T27 | Phases authored AFTER the falsification phase (fix-in-scope follow-ups derived from the ritual) render as a distinct "Follow-up Phases" section below the falsification section — not mixed in with the pre-falsification phases above. | Phase 0 | Phase 8 | planned |
| T28 | Legacy plans with a completed `falsification.md` file (pre-1.27.4) still render the falsification section from the log file via `readFalsificationLog`; no regression when no falsification phase is present in impl.md. | Phase 0 | Phase 8 | planned |
...
- [ ] Add `splitPhasesAroundFalsification(phases: Phase[]): { pre: Phase[]; falsification: Phase | null; post: Phase[] }` helper to `apps/indusk-admin/src/lib/phases.ts`. Detects the falsification phase by title containing `"Falsification"` (case-insensitive substring). If multiple match, uses the first. If none match, returns `{ pre: phases, falsification: null, post: [] }`. Phases before the falsification phase become `pre`; phases after become `post`. Pure, no side effects.
- [ ] Extend `apps/indusk-admin/src/components/PlanDetail.tsx` `PhasesSection` to call `splitPhasesAroundFalsification` and render only the `pre` phases (unchanged UI for normal phases). Add a sibling `FollowUpPhasesSection` component for the `post` phases — same collapsible-phase shape as PhasesSection but with a "Follow-up Phases" section heading and `data-testid="followup-phases-section"`. Render follow-up section only when `post.length > 0` (quiet when no fix-in-scope phases).
- [ ] Enhance `FalsificationSection` in `PlanDetail.tsx`: accept an optional `phase: Phase | null` prop. When `phase` is non-null, render the phase's trajectory rows as "Hypotheses" (one per row, with badges keyed to `state`) and the phase's checklist items as "Fix items" (parsed from the markdown content via `extractChecklistItems`). The section's status badge reads `complete` when all rows are `passing`/`skipped` AND the content has no unchecked items; otherwise `in-progress`. When `phase` is null AND legacy `plan.falsification` is set, keep the existing log-based rendering (backcompat). When both are null, keep the existing "No falsification ritual run for this plan." empty state.
- [ ] Add `extractChecklistItems(markdown: string): Array<{ text: string; checked: boolean }>` helper to `apps/indusk-admin/src/lib/phases.ts`. Simple regex over lines matching `^- \[([ x])\] (.+)`. Used by the falsification-phase rendering to show fix items as a list. Keep scope narrow — not a general markdown AST parser.
- [ ] Update `apps/indusk-admin/src/lib/phases.ts` tests (`apps/indusk-admin/src/lib/phases.test.ts` if it exists, else new) to cover the split helper: (a) falsification phase in middle of list → correct pre/falsification/post split; (b) no falsification phase → all pre, null falsification, empty post; (c) falsification phase at end → pre + falsification + empty post; (d) multiple phases after falsification → all go to post in order.
- [ ] Bump `apps/indusk-mcp/package.json` version → 1.27.6. Add changelog entry under "Fixed" for 1.27.6 naming the admin-UI falsification-phase rendering fix, noting the three sub-changes (split phases, falsification-phase rendering, follow-up phases), and naming the backcompat preservation for legacy `falsification.md` plans.

#### Phase 8 Verification
- [ ] T26 (write red): commit `apps/indusk-admin/src/lib/phases.test.ts` with unit tests for `splitPhasesAroundFalsification` (4 cases per above item) AND for `extractChecklistItems` (happy path + empty + mixed checked/unchecked). Initially red because the helpers don't exist; green after authoring.
- [ ] T27 (write red): extend `apps/indusk-admin/src/components/PlanDetail.test.tsx` with component-level test rendering a PlanDetail whose impl has Phase 7 titled "Falsification" + Phase 8 (regular) + Phase 6 (regular pre). Assert: `data-testid="falsification-section"` contains the hypotheses from Phase 7's trajectory rows AND Phase 7's checklist items as fix items; `data-testid="followup-phases-section"` is present and contains Phase 8; `data-testid="phases-section"` contains only Phases 1-6, NOT Phase 7.
- [ ] T28 (write red): extend the same PlanDetail test with a legacy-flow case: no falsification phase in impl, but `plan.falsification` populated with log entries. Assert the existing falsification rendering (hypotheses from entries, terminator if present) still works. Green from the start — backcompat guard.
- [ ] All Phase 1–7 tests still green. `pnpm --filter indusk-admin vitest run` → no regressions.
