...
| T22 | `daemonStatus()` returns `running: false` when the recorded PID points at a live process that is NOT listening on the recorded port (PID-reuse simulation). Today `isAlive`-only check false-positives and `uiStart` refuses to spawn a second daemon, blaming a stranger's PID. | Phase 0 | Phase 7 | planned |
| T23 | `daemonStop()` does NOT signal a process whose identity can't be confirmed — if the PID is alive but the recorded port is not listening, treat it as stale and clean up without SIGTERM. A spawned noop child sitting at the recorded PID must survive `daemonStop`. | Phase 0 | Phase 7 | planned |
| T24 | `readRegistry()` called against a malformed `projects.json` must quarantine the file (rename to `projects.json.corrupt.{ISO}.bak`) before `addProject()` writes a fresh registry — the malformed bytes must be preserved, not silently overwritten by the next write path. | Phase 0 | Phase 7 | planned |
...
- [ ] Quarantine malformed `projects.json` in `apps/indusk-mcp/src/lib/admin/registry.ts`'s `readRegistry()`: when JSON.parse throws OR shape-validation fails, rename the existing file to `projects.json.corrupt.{ISO-timestamp}.bak` via `renameSync` before returning `emptyRegistry()`. Print a single-line warning to `process.stderr` naming the quarantined path. Any subsequent `writeRegistry` creates a brand-new `projects.json`; the malformed bytes are preserved on disk for user recovery. The quarantine path includes the ISO timestamp (not just `.bak`) so repeated corruption events don't overwrite each other.
- [ ] Extend `resolveOpenPath()` in `apps/indusk-mcp/src/bin/commands/ui.ts` to walk up cwd's parents until it finds a registered project match. For each ancestor (starting at cwd, ending at filesystem root), compare its `realpathSync` against each registered project's `realpathSync`'d `path`. First match wins; return `/p/{name}/`. Cap at 40 `dirname` iterations as a loop sentinel (filesystem paths never exceed this in practice). Fall through to `/` only when the walk-up completes without a hit.
- [ ] Bump `apps/indusk-mcp/package.json` version → 1.27.5 (patch-level hardening). Add a 1.27.5 entry to `apps/indusk-docs/src/changelog.md` naming the four fixes and their rationale: "daemon-identity verification (PID-reuse hardening), registry quarantine on malformed JSON (data-loss prevention), cwd-walk-up in bare `indusk ui` (T16 subdirectory regression fix)".

#### Phase 7 Verification
- [ ] T22 (write red): commit `apps/indusk-mcp/src/lib/admin/__tests__/daemon-identity.test.ts` with a "PID reuse makes status report false-positive running" case. Setup: `INDUSK_HOME=<tmp>`, write `admin-ui.pid` containing `process.pid` (guaranteed alive) and `admin-ui.json` with `port: 65001` (nothing listens there). Assert `await daemonStatus()` returns `{ running: false }`. Red today because `isAlive(process.pid)` is true and meta parses, so status currently returns `running: true`. Green after the `verifyIdentity` rewire.
- [ ] T23 (write red): in the same test file, add a "daemonStop does not SIGTERM a stranger" case. Setup: spawn a detached noop child (`node -e "setInterval(()=>{},60000)"`); write `admin-ui.pid` pointing at the child's PID + meta with port `65001` (child isn't listening). Call `await daemonStop()`, then assert `isAlive(childPid)` is still true 200ms later (no signal was delivered). Red today because `daemonStop` signals any PID whose `isAlive` returns true. Green after the identity gate lands.
- [ ] T24 (write red): commit `apps/indusk-mcp/src/lib/admin/__tests__/registry-quarantine.test.ts`: `INDUSK_HOME=<tmp>`, write `projects.json` with contents `"{not-valid-json"` (malformed). Call `addProject("/tmp/proj-a")`. Assert (a) `projects.json.corrupt.*.bak` exists in the INDUSK_HOME and contains the original malformed bytes byte-for-byte; (b) the new `projects.json` contains exactly one entry (for `/tmp/proj-a`); (c) the old `projects.json` file's malformed content is NOT present in the new `projects.json`. Red today because `readRegistry` silently returns `emptyRegistry()` and the subsequent write overwrites the corrupt bytes with the new single-entry registry — corrupt data vanishes.
- [ ] T25 (write red): commit `apps/indusk-mcp/src/__tests__/resolve-open-path-walkup.test.ts`: `INDUSK_HOME=<tmp>`, register `/tmp/projroot` via `addProject`, create `/tmp/projroot/apps/indusk-mcp/src` on disk, `process.chdir("/tmp/projroot/apps/indusk-mcp/src")`, import `resolveOpenPath` (may require re-exporting it from `ui.ts` as a named export for test access — acceptable surface addition). Assert the return value is `/p/{name}/` (the registered name for `/tmp/projroot`). Red today because exact-match `safeRealpath(p.path) === cwd` fails at subdirectories. Green after walk-up lands.
- [ ] Regression check: all Phase 1–6 tests still green. `pnpm turbo test --filter=@infinitedusky/indusk-mcp` passes; `pnpm --filter indusk-admin vitest run` passes. Subprocess CLI tests (admin-cli-lifecycle, cli-bare-ui-cwd-aware) still green — the identity gate should NOT affect normal start/stop where the daemon is genuinely listening.
