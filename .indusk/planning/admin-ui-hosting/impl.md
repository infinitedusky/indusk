...
- [ ] Extend `apps/indusk-admin/src/components/ui/CollapsibleSection.tsx` with an optional `persistKey?: string` prop. When provided, initial state reads from `localStorage[persistKey]` (value `"1"` = open, `"0"` = closed). Missing key → falls back to `defaultOpen`. On every toggle, `localStorage.setItem(persistKey, newOpen ? "1" : "0")`. Guard `typeof window !== "undefined"` so SSR doesn't crash. If localStorage throws (privacy mode / disabled), swallow and behave like `defaultOpen`.
- [ ] Convert CollapsibleSection to a client component if it isn't already (`"use client"` directive). State needs to be interactive and the localStorage read happens in `useEffect`/initial state — incompatible with pure server rendering. Verify no server-component callers regress (they can still render it; React hydrates client-side).
- [ ] Update every CollapsibleSection usage in `apps/indusk-admin/src/components/PlanDetail.tsx` to pass `persistKey` — string of the form `"plan:{planName}:section:{sectionSlug}"`. Sections: `research`, `brief`, `test-plan`, `adr`, and each phase (`phase-{N}`). Wire `plan.name` through to the section components so they can build the key.
- [ ] Update CollapsibleSection usages in `apps/indusk-admin/src/app/p/[project]/layout.tsx` (if any — Research sidebar group is a candidate) to pass `persistKey` keyed by project + section slug so the sidebar toggles also persist.
- [ ] Bump `apps/indusk-mcp/package.json` version → 1.27.7. Changelog entry under "Added" for 1.27.7 — note localStorage persistence of CollapsibleSection state, per-plan-per-section keys, SSR/privacy-mode safe fallback to `defaultOpen`.

#### Phase 9 Verification
- [ ] T29 (write red): commit `apps/indusk-admin/src/components/ui/CollapsibleSection.test.tsx` (or extend existing) with 3 browser-mode cases — (a) absent `persistKey` falls back to `defaultOpen` (regression guard for current behavior); (b) present `persistKey` + pre-seeded `localStorage[key] = "0"` renders CLOSED even when `defaultOpen: true`; (c) toggling open writes `"1"` to `localStorage[key]`. Red at authoring because the prop doesn't exist; green after the extension lands.
- [ ] T30 (write red): commit `apps/indusk-admin/src/__tests__/collapsible-persistence-integration.test.ts` (or component-level in PlanDetail.test.tsx) with one case — render PlanDetail with a Plan, simulate closing the Brief section (click the chevron), rerender with a fresh PlanDetail for the SAME plan, assert Brief is CLOSED (not re-opened). Red until persistKey is wired through.
- [ ] All Phase 1–8 tests still green after the changes. `cd apps/indusk-admin && pnpm vitest run` → all passing.
