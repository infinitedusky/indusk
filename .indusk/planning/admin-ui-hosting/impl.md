...
- [ ] Extract the tarball into a clean tempdir; spawn `next start` against the bundled `admin/` directory; curl `/`; assert HTTP 200. (This is the portability test for ADR Risk #1 — proves the bundle round-trips through pack/extract intact.)
- [ ] If tarball exceeds 50 MB, profile the `.next/` output to identify large unused features (e.g., image optimization). Disable via `next.config.ts` and re-measure. **Blocker if it can't be brought under 50 MB without removing essential functionality** — escalate to user, may require revisiting variant choice.
