...
| T6 | A search across `apps/indusk-mcp/src/` for `getScm`, `jj.ts`, `NotAJjRepoError`, or `getJjReachable` finds zero matches. | Phase 0 | Phase 4 | passing |
...
- [x] **H1 fix (T14)**: in `apps/indusk-mcp/src/bin/commands/init.ts`, after the `[Config]` block, ran `git rev-parse --git-dir` (cheap fail-fast — exits non-zero if the project isn't in a git tree). On failure, emit a scoped stderr warning naming git as the required SCM + the recovery (`git init` + at least one commit). Init still exits 0 (informational warning, not a hard error).
- [x] **H3 fix (T15)**: wrapped the `runSync` call in `bin/cli.ts`'s `graph sync` action with try/catch + friendly error translation (two cases: "not a git repository" → name `git init` as recovery; "unknown revision / no HEAD" → name `git commit --allow-empty` as recovery; default → raw message). Exits with code 1. The `graph_sync` MCP handler in `tools/graph-tools.ts` already had a try/catch but was returning the raw `Command failed: git rev-parse ...` message — added the same friendly translation there so MCP callers get the same UX.
- [x] **H5 fix (T16)**: in `apps/indusk-mcp/hooks/eval-trigger.js`, tightened `TRIGGER_RE` from `/\bgit commit\b/` to `/\bgit commit(?=$|\s|;|&|\|)/` — lookahead requires the next character after `commit` to be end-of-string, whitespace, or a shell separator. Plumbing commands like `git commit-tree` and `git commit-graph` no longer match. Updated the older T16 "uses word boundaries" assertion to use the left-edge-only pattern (the per-command negative cases live in the new T16 test group).

#### Phase 6 Verification
- [x] T14 passes — `init-no-git-warning.test.ts` exercises both the non-git and git-initialized paths
- [x] T15 passes — `graph-sync-no-git-friendly.test.ts` covers the not-a-git-repo case AND the commitless-repo case; both exit non-zero with friendly stderr and no stack trace
- [x] T16 passes — `eval-trigger-filter-falsepositives.test.ts` gained 4 assertions extracting `TRIGGER_RE` from source and asserting porcelain happy path + 3 negative cases (commit-tree, commit-graph, git committer)
- [x] Full `pnpm vitest run` from `apps/indusk-mcp/` passes — 653 passed, 11 skipped (up from 645 with the 8 new Phase 6 cases)

#### Phase 6 Context
- [x] Updated CLAUDE.md Known Gotchas: added an entry about the eval-trigger regex needing right-edge non-`-` boundary

#### Phase 6 Document
- [x] Appended a Phase 6 entry to the 1.31.0 changelog under `[Unreleased]` (the falsification fixes go out with the same 1.31.0 release rather than a 1.31.1 patch since 1.31.0 hasn't been published yet)
