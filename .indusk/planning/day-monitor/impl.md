...
- [x] Author A25–A30 red at phase start: A25 in `monitor-mark.test.ts` (PATH without `claude`), A26 in `monitor-status.test.ts` and `apps/indusk-admin/src/__tests__/http-promise-health.test.ts` (stub query server), A27 and A30 in `monitor-status.test.ts`, A28 in `monitor-mark.test.ts` (a config without a group id, the hook run from a `git worktree add` checkout), A29 in `monitor-plans.test.ts` through `helpers/plan-worktree-fixture.ts`. Read 2026-09-19, each red on its own assertion: A25 no marked span; A26 the CLI dies on a JSON parse error (exit 1, a stack) and the admin's project pages 500; A27 the aliased mark uncounted; A28 the worktree's mark says `some_plan` where the trunk's says the trunk folder; A29 the trunk's impl gets nothing and the worktree's nothing either; A30 a count of 1500 printed as exact. A26's CLI half spawns asynchronously — its stub answers from the test process, which `spawnSync` blocks
- [x] A25: both evaluator spawns listen for `error`, and a spawn failure resolves as a failed run (`claude could not be started: <message>`) that is logged and marked violated like any other. Each spawn settles once (`close` or `error`), and `stdin` gets an error listener too — writing the prompt to a child that never started raises EPIPE. `evaluator-spans.test.ts`'s fake child gained `stdin.on`, which a real one has
- [x] A26: `getJson` reads and parses the body inside its try, turning a parse failure, an aborted read or an unexpected shape (`data` not an array) into `JaegerUnreachable` naming the URL; the admin's health read treats any failure as unreachable, so a health read can never take down a page. `getJson` became `getData`, which returns the `data` array or throws `JaegerUnreachable` — one place for every way the answer can be wrong
- [x] A27: `markedSpans` asks for a promise's aliases too and counts their marks under the promise's name. `markedSpans({ aliases })` — status, watch and the admin pass the registry's
...
- [x] One call for "this project's marks": `readPromiseMarks(root, registry, { since? })` in `lib/promises/telemetry.ts` picks the behaviour promises that are not retired, their aliases, `markProjectId(root)` and the quiet window, and calls `markedSpans`. `status` (`bin/commands/promises.ts`), `watch` (`lib/promises/watch.ts`) and the admin's `readHealth` each assemble those four by hand today — rule of three; A27 and A28 each had to change all three
- [x] Move `addTrajectoryRow` and `justifyLateRow` from `lib/promises/reopen.ts` to `lib/trajectory/append-row.ts` (exported `appendLateRow`), beside `state-ops.ts`'s `updateRowState` — editing a trajectory table and its register is the trajectory module's job; `reopen.ts` keeps writing the Maintenance phase and calls it. Generalized on the way: the caller passes `{ asserts, why }`, so the module knows nothing about incidents
- [x] `openMaintenancePhasesIn(dir)` in `reopen.ts` — the exists → read → parse of a plan folder's impl that `after-close.ts` and `tools/plan-tools.ts` each spell
- [x] Extract `HealthChip`, `HealthDetail` and `PromiseStateCell` from `apps/indusk-admin/src/components/Promises.tsx` (486 lines, +148 in this plan) into `components/PromiseHealth.tsx` — react's one component per file for the observed-health axis; `Promises.tsx` keeps the table, groups and incidents. `PromiseStateCell` stayed in `Promises.tsx`: it renders the declared-state chip too, and moving it would make the two files import each other; `PromiseHealth.tsx` holds `HealthChip`, `HealthDetail` and `GREY` (74 lines; `Promises.tsx` 486 → 426)
- [x] Move the not-Jaeger stub and its hand-written daemon record, copied in `monitor-status.test.ts` and `http-promise-health.test.ts`, into `helpers/local-jaeger.ts` as `startFakeQueryPort(home, body)` — one home for how a test stands in for the daemon
- [x] Restore `apps/indusk-mcp/src/__tests__/fixtures/lifecycle-parity.snapshot.json` to main's bytes — Build Phase 6's formatter run reflowed it (1,456 lines, whitespace only), which is noise in this plan's diff. The plan's diff no longer touches it; `lifecycle-parity.test.ts` passes on main's bytes
