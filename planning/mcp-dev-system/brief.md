...
- `init` — copies skills to `.claude/skills/`, creates CLAUDE.md structure, sets up `.mcp.json`, creates `planning/` directory
...
- Phase enforcement: validate that implement → verify → context order is followed before advancing
...
The existing `apps/mcp/` scaffold already has `@modelcontextprotocol/sdk` installed and is wired for stdio transport.

## Scope

### In Scope
- MCP server with plan, quality, context, and system tools
- CLI for init and update
- Published as npm package
- Dogfooded in this repo first (the MCP server builds itself using the system it provides)

### Out of Scope
- Document skill integration (planned separately, will be added as a tool later)
- Hosted/remote mode
...
- `npx @infinitedusky/dev-system init` sets up a fresh project with all skills, CLAUDE.md, and MCP config in under 30 seconds
- `list_plans` returns structured data for all plans with correct stages
- `advance_plan` refuses to advance if verification or context items are incomplete
...
- `planning/document-skill/` (will add document tools to the MCP server)
