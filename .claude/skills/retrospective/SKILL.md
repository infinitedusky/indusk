...
5. **Release the assignment, then delete the worktree and the branch** — `indusk worktree release {plan-name}` first, so the admin and the plan tools read the plan from the trunk, which now holds the merged work; then `git worktree remove <worktree-path>`, `git branch -d plan/{plan-name}`, and `git push origin --delete plan/{plan-name}` when it was pushed. Release refuses when the plan has no assignment — a plan whose worktree was made by hand and never assigned; that refusal is the only one to pass over. A worktree removed without the release shows as "assigned worktree … no longer exists" until it is released. A worktree left behind is a stale `.next/` lock, a stale `dist/`, and a `⚠ collision` in the next `indusk agent list`.
