...
describe.skipIf(SHOULD_SKIP)(
	"multi-agent end-to-end — two simulated sessions on one project",
	() => {
		...
		it("two sessions register, both visible in `agent list`, both can come and go independently", () => {
			...
			expect(
				runAs(sessionA, projectDir, ["agent", "register", "--task", "auth refactor"]).status,
			).toBe(0);
			expect(
				runAs(sessionB, projectDir, ["agent", "register", "--task", "telemetry spike"]).status,
			).toBe(0);
