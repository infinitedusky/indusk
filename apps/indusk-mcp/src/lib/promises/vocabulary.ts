...
/** A promise name: kebab-case, so it sits in a file name and a comment unchanged. */
export const PROMISE_NAME = /^[a-z0-9][a-z0-9-]*$/;
...
export function promiseTokenPattern(name: string): RegExp {
	return new RegExp(
		String.raw`\bpromise:\s*${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?![a-z0-9-])`,
	);
}
...
export function anyPromiseTokenPattern(): RegExp {
	return /\bpromise:\s*([a-z0-9][a-z0-9-]*)(?![a-z0-9-])/g;
}
