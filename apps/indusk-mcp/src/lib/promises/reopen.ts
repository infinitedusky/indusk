...
function maintenancePhase(n: number, incidentId: string, otelGate: boolean): string {
	const otel = otelGate
		? [
				`#### Build Phase ${n} OTel`,
				"",
...
export function reopenOwner(planRoot: string, owner: string, incidentId: string): ReopenResult {
	...
	if (!existsSync(implPath)) {
		writeFileSync(
			implPath,
			`---\ntitle: "${owner} — maintenance"\nstatus: in-progress\n---\n\n# ${owner} — maintenance\n\n## Checklist\n\n${maintenancePhase(1, incidentId, otelGate)}`,
		);
	...
	writeFileSync(
		implPath,
		`${text.replace(/\n*$/, "\n")}\n${maintenancePhase(next, incidentId, otelGate)}`,
	);
