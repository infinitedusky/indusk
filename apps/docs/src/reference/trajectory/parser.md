...
| `@infinitedusky/indusk-mcp/impl-parser` | `lib/impl-parser.ts` | `parseImplString` → `ImplPhase {kind, number, ordinal, name, gates}`, `getPhaseCompletion` (its record carries `ref` and `ordinal`), `findPhase(parsed, ref)` |
