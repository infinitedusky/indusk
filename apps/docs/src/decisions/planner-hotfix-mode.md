...
> **Salvaged 2026-09-14** from the unmerged `plan/planner-hotfix-mode-phase-1` branch (tip kept as tag `salvage/planner-hotfix-mode`). The decision below was made and its plan closed in July 2026, but the hotfix workflow it describes never reached main: the planner skill on main has no `hotfix` workflow. Kept as the record of the decision.
