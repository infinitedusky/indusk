...
1. **Find the changed files that deserve scrutiny.** Call `listOversizedChangedFiles(planRoot, baseRef)` from `apps/indusk-mcp/src/lib/cleanup/oversized.js`. It diffs the plan's branch against its merge-base and returns changed files over their resolved cap as `{ path, loc, cap, scope, isNew }`. Caps come from the `cleanup` config block (`max_file_loc` default 400 + per-scope overrides). Flagged means "look here," not "this fails."
...
The phase title **must contain "Cleanup"** — the retrospective gate's `isCleanupComplete` detects the phase by the word "cleanup" (case-insensitive), exactly as the admin UI detects the falsification phase.
