...
- **Workbench-shaped projects**: `.indusk/current.md` lives at the workbench root. Since versioned workbenches (1.37.0) that root is a git repository of its own, with its own remote, and `merge=union` applies to `current.md` there exactly as in a flat project — when two *machines* push concurrent appends. Between two concurrent processes on *one* machine there is still no git layer, only the filesystem, so the load-bearing concurrency primitive remains the file lock that the agent CLI and the MCP tool hold around every read-modify-write: `<projectRoot>/.indusk/current.md.lock` opened with `O_EXCL`, 5s acquisition timeout, 30s stale-lock TTL. (Before 1.37.0 the root was not a repository at all, and the lock was the only primitive.)
