...
import type { PromiseEntry, Registry } from "@infinitedusky/indusk-mcp/promises/registry";
import {
	JaegerUnreachable,
...
export const PROMISE_HEALTHS = ["red", "green", "unverified", "amber", "grey"] as const;
...
export async function readHealth(projectRoot: string, registry: Registry): Promise<HealthRead> {
	...
	try {
		const marks = await markedSpans({
			promises: behaviour,
			since: new Date(Date.now() - getQuietWindowDays(projectRoot) * 86_400_000),
 ...
 */
export function healthOf(p: PromiseEntry, read: HealthRead | null): HealthRow | null {
...
export function healthRows(registry: Registry, read: HealthRead | null): Record<string, HealthRow> {
...
export function redPlans(registry: Registry, rows: Record<string, HealthRow>): Set<string> {
	return new Set(
