import { layout } from "./chunk-HTVY2ZTW.js";
import { Graph } from "./chunk-KAQAGEVX.js";
import {
	clear,
	common$1,
	configureSvgSize,
...
// ../../node_modules/.pnpm/mermaid@10.9.5/node_modules/mermaid/dist/requirementDiagram-08caed73.js
...
var parser = (() => {
	...
	var parser2 = {
		...
		table: [
			...
			{ 19: [2, 53] },
			{ 19: [2, 54] },
			{ 52: 46, 55: $Vi, 56: $Vj, 57: $Vk, 58: $Vl, 59: $Vm, 60: $Vn, 61: $Vo },
			{ 52: 54, 55: $Vi, 56: $Vj, 57: $Vk, 58: $Vl, 59: $Vm, 60: $Vn, 61: $Vo },
			{ 5: [1, 55] },
			{ 5: [1, 56] },
			{ 53: [1, 57] },
			o($Vp, [2, 40]),
			...
			o($Vp, [2, 46]),
			{ 54: [1, 58] },
			{ 5: $Vq, 20: 59, 21: $Vr, 24: $Vs, 26: $Vt, 28: $Vu, 30: $Vv },
			{ 5: $Vw, 30: $Vx, 46: 66, 47: $Vy, 49: $Vz },
			{ 23: 71, 62: $Ve, 63: $Vf },
			{ 23: 72, 62: $Ve, 63: $Vf },
			o($VA, [2, 13]),
			{ 22: [1, 73] },
			...
			{ 22: [1, 75] },
			{ 22: [1, 76] },
			{ 5: $Vq, 20: 77, 21: $Vr, 24: $Vs, 26: $Vt, 28: $Vu, 30: $Vv },
			o($VA, [2, 19]),
			o($VA, [2, 33]),
			{ 22: [1, 78] },
			{ 22: [1, 79] },
			{ 5: $Vw, 30: $Vx, 46: 80, 47: $Vy, 49: $Vz },
			o($VA, [2, 37]),
			o($VA, [2, 38]),
			o($VA, [2, 39]),
			{ 23: 81, 62: $Ve, 63: $Vf },
			...
			{ 5: [2, 57] },
			{ 5: [2, 58] },
			{ 5: $Vq, 20: 106, 21: $Vr, 24: $Vs, 26: $Vt, 28: $Vu, 30: $Vv },
			{ 5: $Vq, 20: 107, 21: $Vr, 24: $Vs, 26: $Vt, 28: $Vu, 30: $Vv },
			{ 5: $Vq, 20: 108, 21: $Vr, 24: $Vs, 26: $Vt, 28: $Vu, 30: $Vv },
			{ 5: $Vq, 20: 109, 21: $Vr, 24: $Vs, 26: $Vt, 28: $Vu, 30: $Vv },
			{ 5: $Vw, 30: $Vx, 46: 110, 47: $Vy, 49: $Vz },
			{ 5: $Vw, 30: $Vx, 46: 111, 47: $Vy, 49: $Vz },
			o($VA, [2, 14]),
			o($VA, [2, 15]),
			o($VA, [2, 16]),
			o($VA, [2, 17]),
	...
	var lexer = /* @__PURE__ */ (() => {
		var lexer2 = {
			...
			conditions: {
				...
				string: { rules: [48, 49], inclusive: false },
				INITIAL: {
					rules: [
...
var insertLineEndings = (parentNode, conf2) => {
	const containsNode = parentNode
		.append("defs")
		.append("marker")
		.attr("id", ReqMarkers.CONTAINS + "_line_ending")
		.attr("refX", 0)
		.attr("refY", conf2.line_height / 2)
		.attr("markerWidth", conf2.line_height)
		.attr("markerHeight", conf2.line_height)
		.attr("orient", "auto")
		.append("g");
	containsNode
		.append("circle")
		.attr("cx", conf2.line_height / 2)
	...
	containsNode
		.append("line")
		.attr("x1", 0)
		.attr("x2", conf2.line_height)
		.attr("y1", conf2.line_height / 2)
		.attr("y2", conf2.line_height / 2)
		.attr("stroke-width", 1);
	containsNode
		.append("line")
		.attr("y1", 0)
		.attr("y2", conf2.line_height)
		.attr("x1", conf2.line_height / 2)
	...
	parentNode
		...
		.attr("refX", conf2.line_height)
		.attr("refY", 0.5 * conf2.line_height)
		.attr("markerWidth", conf2.line_height)
		.attr("markerHeight", conf2.line_height)
		.attr("orient", "auto")
		.append("path")
...
var newTitleNode = (parentNode, id, txts) => {
	...
	const yPadding = 1.5 * conf.rect_padding;
	const linePadding = i * conf.line_height * 0.75;
	const totalY = yPadding + linePadding;
	parentNode
		.append("line")
...
var newBodyNode = (parentNode, id, txts, yStart) => {
	...
	txts.forEach((textStr) => {
		...
		while (currentTextLen > charLimit && currentRow < 3) {
			const firstPart = textStr.substring(0, charLimit);
		...
		if (currentRow == 3) {
			const lastStr = wrappedTxts[wrappedTxts.length - 1];
...
var drawRelationshipFromLayout = (svg, rel, g, insert, diagObj) => {
...
var drawReqs = (reqs, graph, svgNode) => {
	Object.keys(reqs).forEach((reqName) => {
		const req = reqs[reqName];
		reqName = elementString(reqName);
		log$1.info("Added new requirement: ", reqName);
		const groupNode = svgNode.append("g").attr("id", reqName);
		const textId = "req-" + reqName;
		const rectNode = newRectNode(groupNode, textId);
		const titleNodeInfo = newTitleNode(groupNode, reqName + "_title", [
			`<<${req.type}>>`,
			`${req.name}`,
		]);
		newBodyNode(
...
var drawElements = (els, graph, svgNode) => {
	Object.keys(els).forEach((elName) => {
		...
		const rectNode = newRectNode(groupNode, textId);
		const titleNodeInfo = newTitleNode(groupNode, textId + "_title", [`<<Element>>`, `${elName}`]);
		newBodyNode(
			groupNode,
			textId + "_body",
...
var addRelationships = (relationships, g) => {
	relationships.forEach((r) => {
		const src = elementString(r.src);
		const dst = elementString(r.dst);
		g.setEdge(src, dst, { relationship: r });
	});
	return relationships;
};
var adjustEntities = (svgNode, graph) => {
...
var draw = (text, id, _version, diagObj) => {
	...
	const g = new Graph({
		...
		.setDefaultEdgeLabel(() => ({}));
	const requirements2 = diagObj.db.getRequirements();
	const elements2 = diagObj.db.getElements();
	const relationships = diagObj.db.getRelationships();
...
export { diagram };
//# sourceMappingURL=requirementDiagram-08caed73-N3SFCZGB.js.map
