import { decodeEntities, dedent, getConfig$1, log$1, sanitizeText$2 } from "./chunk-MVEA22LD.js";
			...
			function matchesTemplate(search2, template) {
				...
				while (searchIndex < search2.length) {
					if (
						...
						(template[templateIndex] === search2[searchIndex] || template[templateIndex] === "*")
					) {
...
Diff.prototype = {
	diff: function diff(oldString, newString) {
		...
		var maxExecutionTime =
			(_options$timeout = options.timeout) !== null && _options$timeout !== void 0
				? _options$timeout
				: Infinity;
...
function arrays(input, expect) {
	const arr = diffArrays(input, expect);
	let i = 0,
		j = 0,
		k = 0,
		tmp,
...
function lines(input, expect, linenum = 0) {
	let i = 0,
		tmp,
		output = "";
	const arr = diffLines(input, expect);
...
function chars(input, expect) {
	const arr = diffChars(input, expect);
	let i = 0,
		output = "",
		tmp;
	let l1 = input.length;
...
function sort(input, expect) {
	...
	if (isArr) {
		...
		for (; i < keys.length; i++) {
			if (Object.hasOwn(input, (k = keys[i]))) {
...
not.type = (val, exp, msg) => {
	const tmp = typeof val;
	assert(tmp !== exp, tmp, exp, "not.type", false, `Expected "${tmp}" not to be "${exp}"`, msg);
};
not.instance = (val, exp, msg) => {
...
not.snapshot = (val, exp, msg) => {
	val = dedent2(val);
	exp = dedent2(exp);
	assert(val !== exp, val, exp, "not.snapshot", false, "Expected value not to match snapshot", msg);
};
not.fixture = (val, exp, msg) => {
	val = dedent2(val);
	exp = dedent2(exp);
	assert(val !== exp, val, exp, "not.fixture", false, "Expected value not to match fixture", msg);
};
not.match = (val, exp, msg) => {
...
function subtokenize(events) {
	...
	while (++index2 < events.length) {
		...
		if (
			...
			events[index2 - 1][1].type === types.listItemPrefix
		) {
...
function tokenizeLabelEnd(effects, ok2, nok) {
	...
	while (index2--) {
		if (
			(self.events[index2][1].type === types.labelImage ||
				self.events[index2][1].type === types.labelLink) &&
...
function tokenizeSetextUnderline(effects, ok2, nok) {
	...
	function start(code) {
		...
		while (index2--) {
			if (
				self.events[index2][1].type !== types.lineEnding &&
				self.events[index2][1].type !== types.linePrefix &&
				self.events[index2][1].type !== types.content
			) {
...
function createTokenizer(parser, initialize, from) {
	let point3 = Object.assign(from ? Object.assign({}, from) : { line: 1, column: 1, offset: 0 }, {
		_index: 0,
		_bufferIndex: -1,
	});
	const columnStart = {};
		...
		while (++index2 < events.length) {
			...
			if (own.call(handler, events[index2][1].type)) {
				handler[events[index2][1].type].call(
					Object.assign({ sliceSerialize: events[index2][2].sliceSerialize }, context),
					events[index2][1],
				);
			}
		}
...
function addHtmlSpan(element2, node2, width, classes, addBackground = false) {
    ...
    <span class="${labelClass} ${classes}" ` +
				(node2.labelStyle ? 'style="' + node2.labelStyle + '"' : "") +
				">" +
				label +
				"</span>",
			getConfig$1(),
...
//# sourceMappingURL=chunk-SOBCWSDB.js.map
