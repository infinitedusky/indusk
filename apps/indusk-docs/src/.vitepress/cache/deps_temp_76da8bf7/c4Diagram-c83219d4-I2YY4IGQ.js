import { drawRect, getNoteRect } from "./chunk-A4DEKYAK.js";
import {
	assignWithDepth$1,
	calculateTextHeight,
	calculateTextWidth,
...
var c4Type;
var getC4Type = () => c4Type;
var setC4Type = (c4TypeParam) => {
	const sanitizedText = sanitizeText$2(c4TypeParam, getConfig());
	c4Type = sanitizedText;
};
var addRel = (type, from, to, label, techn, descr, sprite, tags, link) => {
	if (
		type === void 0 ||
		type === null ||
		from === void 0 ||
...
var addPersonOrSystem = (typeC4Shape, alias, label, descr, sprite, tags, link) => {
...
var addContainer = (typeC4Shape, alias, label, techn, descr, sprite, tags, link) => {
	if (alias === null || label === null) {
		return;
	}
	let container = {};
...
var addComponent = (typeC4Shape, alias, label, techn, descr, sprite, tags, link) => {
	if (alias === null || label === null) {
		return;
	}
	let component = {};
...
var addPersonOrSystemBoundary = (alias, label, type, tags, link) => {
	if (alias === null || label === null) {
		return;
	}
	let boundary = {};
...
var addContainerBoundary = (alias, label, type, tags, link) => {
	if (alias === null || label === null) {
		return;
	}
	let boundary = {};
...
var addDeploymentNode = (nodeType, alias, label, type, descr, sprite, tags, link) => {
	if (alias === null || label === null) {
		return;
	}
	let boundary = {};
...
var updateRelStyle = (typeC4Shape, from, to, textColor, lineColor, offsetX, offsetY) => {
...
var updateLayoutConfig = (typeC4Shape, c4ShapeInRowParam, c4BoundaryInRowParam) => {
...
var getC4ShapeInRow = () => c4ShapeInRow$1;
var getC4BoundaryInRow = () => c4BoundaryInRow$1;
var getCurrentBoundaryParse = () => currentBoundaryParse;
var getParentBoundaryParse = () => parentBoundaryParse;
var getC4ShapeArray = (parentBoundary) => {
...
var getC4Shape = (alias) => c4ShapeArray.find((personOrSystem) => personOrSystem.alias === alias);
var getC4ShapeKeys = (parentBoundary) => Object.keys(getC4ShapeArray(parentBoundary));
var getBoundaries = (parentBoundary) => {
	if (parentBoundary === void 0 || parentBoundary === null) {
		return boundaries;
	} else {
...
var setTitle = (txt) => {
	const sanitizedText = sanitizeText$2(txt, getConfig());
	title = sanitizedText;
};
var db = {
	addPersonOrSystem,
...
var drawImage = (elem, width, height, x, y, link) => {
	const imageElem = elem.append("image");
	imageElem.attr("width", width);
	imageElem.attr("height", height);
	imageElem.attr("x", x);
	imageElem.attr("y", y);
	const sanitizedLink = link.startsWith("data:image/png;base64")
		? link
...
var drawRels$1 = (elem, rels2, conf2) => {
	...
	for (const rel of rels2) {
		...
		const offsetX = rel.offsetX ? parseInt(rel.offsetX) : 0;
		const offsetY = rel.offsetY ? parseInt(rel.offsetY) : 0;
		const url = "";
		if (i === 0) {
...
var drawBoundary$1 = (elem, boundary, conf2) => {
...
var drawC4Shape = (elem, c4Shape, conf2) => {
	var _a;
	...
	const c4ShapeFontConf = getC4ShapeFont(conf2, c4Shape.typeC4Shape.text);
	c4ShapeElem
		.append("text")
		.attr("fill", fontColor)
...
function calcC4ShapeTextWH(textType, c4Shape, c4ShapeTextWrap, textConf, textLimitWidth) {
	if (!c4Shape[textType].width) {
		if (c4ShapeTextWrap) {
			...
			const lines = c4Shape[textType].text.split(common$1.lineBreakRegex);
...
var drawBoundary = (diagram2, boundary, bounds) => {
	...
	const textLimitWidth = calculateTextWidth(boundary.label.text, boundaryLabelConf);
	calcC4ShapeTextWH("label", boundary, boundaryTextWrap, boundaryLabelConf, textLimitWidth);
	svgDraw.drawBoundary(diagram2, boundary, conf);
};
var drawC4ShapeArray = (currentBounds, diagram2, c4ShapeArray2, c4ShapeKeys) => {
	let Y = 0;
	for (const c4ShapeKey of c4ShapeKeys) {
		...
		if (c4Shape.type && c4Shape.type.text !== "") {
			...
			const c4ShapeTypeConf2 = c4ShapeFont(conf, c4Shape.typeC4Shape.text);
...
var getIntersectPoint = (fromNode, endPoint) => {
	const x1 = fromNode.x;
	const y1 = fromNode.y;
	const x2 = endPoint.x;
	const y2 = endPoint.y;
...
var getIntersectPoints = (fromNode, endNode) => {
...
var drawRels = (diagram2, rels2, getC4ShapeObj, diagObj) => {
	let i = 0;
	for (const rel of rels2) {
		i = i + 1;
...
function drawInsideBoundary(
	...
	for (const [i, currentBoundary] of currentBoundaries.entries()) {
		...
		const currentPersonOrSystemArray = diagObj.db.getC4ShapeArray(currentBoundary.alias);
		const currentPersonOrSystemKeys = diagObj.db.getC4ShapeKeys(currentBoundary.alias);
		...
		const nextCurrentBoundaries = diagObj.db.getBoundarys(parentBoundaryAlias);
...
var draw = (_text, id, _version, diagObj) => {
	conf = getConfig().c4;
	...
	const currentBoundaries = diagObj.db.getBoundarys("");
...
export { diagram };
//# sourceMappingURL=c4Diagram-c83219d4-I2YY4IGQ.js.map
