...
import { clone_default, Graph, isUndefined_default, map_default } from "./chunk-KAQAGEVX.js";
import { evaluate, getConfig, log$1, select_default } from "./chunk-MVEA22LD.js";
import {
	clear,
	clear$1,
	createLabel$1,
...
// ../../node_modules/.pnpm/mermaid@10.9.5/node_modules/mermaid/dist/index-5325376f.js
var clusterDb = {};
var descendants = {};
var parents = {};
var clear$12 = () => {
...
var extractor = (graph, depth) => {
	...
	for (const node of nodes) {
		...
		if (!clusterDb[node]) {
			...
			!clusterDb[node].externalConnections && // !graph.parent(node) &&
			graph.children(node) &&
...
var rect = (parent, node) => {
	...
	node.intersect = (point) => intersectRect$1(node, point);
	return shapeSvg;
};
...
var roundedWithTitle = (parent, node) => {
	...
	const text = label
		.node()
		.appendChild(createLabel$1(node.labelText, node.labelStyle, void 0, true));
	let bbox = text.getBBox();
	if (evaluate(siteConfig.flowchart.htmlLabels)) {
		const div = text.children[0];
		const dv = select_default(text);
...
var recursiveRender = async (_elem, graph, diagramType, id, parentCluster, siteConfig) => {
	...
	sortNodesByHierarchy(graph).forEach((v) => {
		const node = graph.node(v);
...
export { render };
//# sourceMappingURL=chunk-VW2L7BRW.js.map
