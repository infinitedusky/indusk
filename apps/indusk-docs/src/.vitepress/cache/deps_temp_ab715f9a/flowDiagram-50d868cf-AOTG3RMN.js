...
var createClusters = (selection, g) => {
	var clusters = g.nodes().filter((v) => isSubgraph(g, v));
	var svgClusters = selection.selectAll("g.cluster").data(clusters, (v) => v);
...
var createEdgeLabels = (selection, g) => {
	var svgEdgeLabels = selection
		.selectAll("g.edgeLabel")
		.data(g.edges(), (e) => edgeToId(e))
		.classed("update", true);
	...
	svgEdgeLabels.each(function (e) {
		...
		if (!Object.hasOwn(edge, "width")) {
			edge.width = bbox.width;
		}
		if (!Object.hasOwn(edge, "height")) {
...
var createEdgePaths = (selection, g, arrows2) => {
...
function createLine(edge, points) {
	var line = (line_default || svg.line)()
		.x((d) => d.x)
		.y((d) => d.y);
...
function enter(svgPaths, g) {
	...
	svgPathsEnter
		...
		.attr("d", (e) => {
			...
			var points = range_default(edge.points.length).map(() => getCoords(sourceElem));
			return createLine(edge, points);
		});
...
var createNodes = (selection, g, shapes2) => {
	var simpleNodes = g.nodes().filter((v) => !isSubgraph(g, v));
	var svgNodes = selection
		.selectAll("g.node")
		.data(simpleNodes, (v) => v)
		.classed("update", true);
	...
	svgNodes.each(function (v) {
		...
		if (Object.hasOwn(node, "width")) {
			bbox.width = node.width;
		}
		if (Object.hasOwn(node, "height")) {
...
function positionEdgeLabels(selection, g) {
	...
	function translate(e) {
		var edge = g.edge(e);
		return Object.hasOwn(edge, "x") ? "translate(" + edge.x + "," + edge.y + ")" : "";
	}
	created.attr("transform", translate);
	applyTransition(selection, g).style("opacity", 1).attr("transform", translate);
}
...
function ellipse(parent, bbox, node) {
	...
	var shapeSvg = parent
		...
		.attr("rx", rx)
		.attr("ry", ry);
	node.intersect = (point) => intersectEllipse(node, rx, ry, point);
	return shapeSvg;
}
function circle(parent, bbox, node) {
	...
	var shapeSvg = parent
		...
		.attr("y", -bbox.height / 2)
		.attr("r", r);
	node.intersect = (point) => intersectCircle(node, r, point);
	return shapeSvg;
}
function diamond(parent, bbox, node) {
	...
	node.intersect = (p) => intersectPolygon(node, points, p);
	return shapeSvg;
}
...
function preProcessGraph(g) {
	g.nodes().forEach((v) => {
		const node = g.node(v);
		if (!Object.hasOwn(node, "label") && !g.children(v).length) {
			node.label = v;
		}
		if (Object.hasOwn(node, "paddingX")) {
			defaults_default(node, {
				paddingLeft: node.paddingX,
				paddingRight: node.paddingX,
			});
		}
		if (Object.hasOwn(node, "paddingY")) {
			defaults_default(node, {
				paddingTop: node.paddingY,
				paddingBottom: node.paddingY,
			});
		}
		if (Object.hasOwn(node, "padding")) {
			defaults_default(node, {
...
function postProcessGraph(g) {
	g.nodes().forEach((v) => {
		var node = g.node(v);
		if (Object.hasOwn(node, "_prevWidth")) {
			node.width = node._prevWidth;
		} else {
			delete node.width;
		}
		if (Object.hasOwn(node, "_prevHeight")) {
...
function question(parent, bbox, node) {
	...
	node.intersect = (point) => intersectPolygon(node, points, point);
	return shapeSvg;
}
...
var addVertices = async (vert, g, svgId, root, _doc, diagObj) => {
	...
	for (const id of keys) {
		...
		const vertexText = vertex.text !== void 0 ? vertex.text : vertex.id;
		let vertexNode;
...
var addEdges = async (edges, g, diagObj) => {
	let cnt = 0;
	let defaultStyle;
...
export { diagram };
//# sourceMappingURL=flowDiagram-50d868cf-AOTG3RMN.js.map
