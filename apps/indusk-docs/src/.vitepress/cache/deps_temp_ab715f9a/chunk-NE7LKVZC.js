...
import { createText } from "./chunk-SOBCWSDB.js";

// ../../node_modules/.pnpm/mermaid@10.9.5/node_modules/mermaid/dist/edges-96097737.js
...
var createLabel = (_vertexText, style, isTitle, isNode) => {
	...
	if (evaluate(config.flowchart.htmlLabels)) {
		...
		const vertexNode = addHtmlLabel(node, config);
		return vertexNode;
	} else {
...
var note = async (parent, node) => {
	...
	node.intersect = (point2) => intersect.rect(node, point2);
	return shapeSvg;
};
var note$1 = note;
...
var choice = (parent, node) => {
	...
	const choice2 = shapeSvg
		.insert("polygon", ":first-child")
		.attr("points", points.map((d) => d.x + "," + d.y).join(" "));
	choice2.attr("class", "state-start").attr("r", 7).attr("width", 28).attr("height", 28);
	node.width = 28;
	node.height = 28;
	node.intersect = (point2) => intersect.circle(node, 14, point2);
	return shapeSvg;
};
var hexagon = async (parent, node) => {
	...
	node.intersect = (point2) => intersect.polygon(node, points, point2);
	return shapeSvg;
};
...
var start = (parent, node) => {
	...
	updateNodeBounds(node, circle2);
	node.intersect = (point2) => intersect.circle(node, 7, point2);
	return shapeSvg;
};
...
var class_box = (parent, node) => {
	...
	const interfaceLabel = labelContainer
		.node()
		.appendChild(createLabel$1(interfaceLabelText, node.labelStyle, true, true));
	...
	const classTitleLabel = labelContainer
		.node()
		.appendChild(createLabel$1(classTitleString, node.labelStyle, true, true));
	...
	maxHeight += lineHeight;
	if (hasInterface) {
		const diffX2 = (maxWidth - interfaceBBox.width) / 2;
...
var insertEdge = (elem, e, edge, clusterDb, diagramType, graph, id) => {
...
//# sourceMappingURL=chunk-NE7LKVZC.js.map
